...
import pyvast
from pyvast import abi, ecc, libvast, vast_link
...
class VAST2PVASTAction(Action):
    def __init__(self, key, data):
        super().__init__('vast2pvast', '.fungible', key, data)
...
def get_action_from_abi_json(action, abi_json, domain=None, key=None):
    ...
    elif action == 'vast2pvast':
        return VAST2PVASTAction(abi_dict['number'].split('#')[1], _bin)
...
class ActionGenerator:
    ...
    def vast2pvast(self, _from, to, number, memo):
        abi_json = base.VAST2PVASTAbi(
            _from=str(_from), to=str(to), number=number, memo=memo)
        return get_action_from_abi_json('vast2pvast', abi_json.dumps())
    ...
    def everipass(self, link):
        everipass = vast_link.VastLink.parse_from_vastli(link)
    ...
    def everipay(self, payee, number, link):
        everipay = vast_link.VastLink.parse_from_vastli(link)
