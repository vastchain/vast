...
from .address import *
from .ecc import *
from .vast_link import *


class TestPyVAST(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ver = libvast.init_lib()
        assert ver == abi.version()
        print('VAST Api Version:', ver)

    def test_vastecc(self):
    ...
    def test_vastabi(self):
        j = r'''
        {
            "name": "test",
            "creator": "VAST8MGU4aKiVzqMtWi9zLpu8KuTHZWjQQrX475ycSxEkLd6aBpraX",
            "issue": {
                "name": "issue",
                "threshold": 1,
                "authorizers": [{
    ...
    def test_vastaddress(self):
        reserved_addr = Address.reserved()
        self.assertEqual(
            'VAST00000000000000000000000000000000000000000000000000', reserved_addr.to_string())
        self.assertEqual('reserved', reserved_addr.get_type())

        pub_key = PublicKey.from_string(
            'VAST6bMPrzVm77XSjrTfZxEsbAuWPuJ9hCqGRLEhkTjANWuvWTbwe3')
        public_key_addr = Address.public_key(pub_key)
        self.assertEqual(
            'VAST6bMPrzVm77XSjrTfZxEsbAuWPuJ9hCqGRLEhkTjANWuvWTbwe3', public_key_addr.to_string())
    ...
    def test_vastlink(self):
        ...
        vast_link = VastLink.parse_from_vastli(link_str)
        header = vast_link.get_header()
        timestamp = vast_link.get_segment_int('timestamp')
        domain = vast_link.get_segment_str('domain')
        token = vast_link.get_segment_str('token')

        signs = vast_link.get_signatures()
        ...
        pub_key, priv_key = generate_new_pair()
        vast_link_1 = VastLink()
        vast_link_1.set_header(3)
        vast_link_1.add_segment_int('timestamp', 1532465234)
        vast_link_1.add_segment_str('domain', 'nd1532465232490')
        vast_link_1.add_segment_str('token', 'tk3064930465.8381')
        vast_link_1.sign(priv_key)

        self.assertEqual(vast_link_1.get_segment_int('timestamp'), 1532465234)
        self.assertEqual(vast_link.get_segment_str('domain'), 'nd1532465232490')

        vast_link = VastLink.parse_from_vastli('0DFYZXZO9-:Y:JLF*3/4JCPG7V1346OZ:R/G2M93-2L*BBT9S0YQ0+JNRIW95*HF*94J0OVUN$KS01-GZ-N7FWK9_FXXJORONB7B58VU9Z2MZKZ5*:NP3::K7UYKD:Y9I1V508HBQZK2AE*ZS85PJZ2N47/41LQ-MZ/4Q6THOX**YN0VMQ*3/CG9-KX2:E7C-OCM*KJJT:Z7640Q6B*FWIQBYMDPIXB4CM:-8*TW-QNY$$AY5$UA3+N-7L/ZSDCWO1I7M*3Q6*SMAYOWWTF5RJAJ:NG**8U5J6WC2VM5Z:OLZPVJXX*12I*6V9FL1HX095$5:$*C3KGCM3FIS-WWRE14E:7VYNFA-3QCH5ULZJ*CRH91BTXIK-N+J1')
        link_id = vast_link.get_link_id()
        self.assertEqual(link_id[0], 139)
        self.assertEqual(link_id[1], 90)
        self.assertEqual(link_id[15], 185)

        vast_link.set_link_id(bytes(
            [139-1, 90, 90, 91, 249, 106, 190, 191, 63, 143, 113, 132, 245, 34, 161, 185]))
        self.assertEqual(vast_link.get_link_id()[0], 138)

        vast_link.set_link_id_rand()
