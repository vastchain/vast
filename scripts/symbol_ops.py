...
@click.option('--bucket', '-b', default='vast-symbols')
@click.option('--ref', '-r', default='vast')
...
def upload(folder, bucket, ref, aws_key, aws_secret):
    ...
    files = []
    for f in get_files(pathlib.Path(folder) / 'vastd'):
        files.append(f)
    for f in get_files(pathlib.Path(folder) / 'vastc'):
        files.append(f)
    for f in get_files(pathlib.Path(folder) / 'vastwd'):
        files.append(f)
