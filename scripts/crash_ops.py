...
@click.option('--bucket', '-b', default='vast-crashes')
...
@click.option('--temp', '-f', default='/tmp/vast-symbols')
...
def analysis(filename, type, temp, output, bucket, aws_key, aws_secret):
    ...
    match = re.search(r'vastd, (\w+)', r.stdout.decode('utf-8'))
    if match is None:
        click.echo('Cannot find checksum of vastd, failed')
        exit(-1)
    ...
    f1 = '{}/vastd/{}/vastd.dbg'.format(folder, checksum)
    f2 = '{}/vastd/{}/vastd.sym'.format(folder, checksum)

    if not os.path.exists(temp):
        os.mkdir(temp)

    folder1 = os.path.join(temp, 'vastd')
    ...
    o1 = os.path.join(folder2, 'vastd.dbg')
    o2 = os.path.join(folder2, 'vastd.sym')

    try:
        if not os.path.exists(o1):
            click.echo('Fetching {} to {}...'.format(f1, o1))
            s3.Bucket('vast-symbols').download_file(f1, o1)

        if not os.path.exists(o2):
            click.echo('Fetching {} to {}...'.format(f2, o2))
            s3.Bucket('vast-symbols').download_file(f2, o2)
    except ClientError as e:
        if e.response['Error']['Code'] == '404':
            click.echo(
                'Cannot find symbol files for vastd with checksum: {}.'.format(checksum))
        else:
            raise
