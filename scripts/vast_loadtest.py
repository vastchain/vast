...
@click.option('--vast_host', help='the host of the string', type=str, default='http://vastd_0:8888')
...
def begin(max_user, increase_rate, run_time, vast_host, log_dir, config_file, traffic_data_dir, work_thread, regin_number, slave_number):
    i = 0
    f = open(config_file)
    line = f.readline()
    while(line):
        ...
        for j in range(0, slave_number):
            ...
            if(not os.path.exists(path)):
                os.mkdir(path, 0755)
            run_the_traffic(i, vast_host, max_user,
                            regin_number, work_thread, path, line)
        print('begin run the loadtest index:'+str(i))
        run_the_loadtest(i, vast_host, log_dir, max_user, increase_rate, run_time, os.path.join(
