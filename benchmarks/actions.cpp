...
static std::unique_ptr<vast::testing::tester>
create_tester() {
    using namespace vast::testing;

    fc::logger::get().set_log_level(fc::log_level(fc::log_level::error));

    auto dir = fc::path("/tmp/vast_benchmarks");
    if(fc::exists(dir)) {
        fc::remove_all(dir);
    }
...
get_nonce_key(const char* prefix) {
    ...
    return vast::testing::tester::get_public_key(name(n));
}
...
get_trx_meta(controller& control, const action& act, const std::vector<name>& auths) {
    ...
    for(auto& auth : auths) {
        auto privkey = vast::testing::tester::get_private_key(auth);
...
auto&
get_exec_ctx() {
    static auto exec_ctx = vast_execution_context_mock();
    return exec_ctx;
}
...
BM_Action_newdomain(benchmark::State& state) {
    ...
    nd.creator  = vast::testing::tester::get_public_key("vast");
    auto auths  = std::vector<name>{N(vast)};
...
BM_Action_newgroup(benchmark::State& state) {
    ...
    ng.group.key_ = vast::testing::tester::get_public_key("vast");
    auto auths    = std::vector<name>{N(vast)};
...
BM_Action_newfungible(benchmark::State& state) {
    ...
    nf.creator  = vast::testing::tester::get_public_key("vast");
    auto auths  = std::vector<name>{N(vast)};
    for(auto _ : state) {
        state.PauseTiming();
...
BM_Action_transferft(benchmark::State& state) {
    ...
    auto nf     = var.as<newfungible>();
    nf.creator  = vast::testing::tester::get_public_key("vast");
    nf.issue.authorizers[0].ref.set_account(vast::testing::tester::get_public_key("vast"));
    nf.name         = get_nonce_sym();
    nf.sym_name     = get_nonce_sym();
    nf.sym          = symbol::from_string(string("5,S#") + get_nonce_sym());
    nf.total_supply = asset::from_string(string("100.00000 S#") + std::to_string(nf.sym.id()));
    ...
    isf.address = address(vast::testing::tester::get_public_key("vast"));
    auto isfact = action(N128(.fungible), (name128)std::to_string(nf.sym.id()), isf);

    tester->push_action(std::move(isfact), auths, address());

    auto tf   = transferft();
    tf.from   = vast::testing::tester::get_public_key("vast");
...
static void
BM_Action_vast2pvast(benchmark::State& state) {
    auto tester = create_tester();

    auto auths = std::vector<name>{N(vast)};
    tester->add_money(address(vast::testing::tester::get_public_key(N(vast))), asset(10000000, vast_sym()));

    auto e2p   = vast2pvast();
    e2p.from   = address(vast::testing::tester::get_public_key(N(vast)));
    ...
    for(auto _ : state) {
        ...
        auto e2pact   = action(N128(.fungible), std::to_string(vast_sym().id()), e2p);
...
BENCHMARK(BM_Action_vast2pvast);
...
BM_Action_fungible_addmeta(benchmark::State& state) {
    ...
    nf.creator  = vast::testing::tester::get_public_key("vast");
    nf.issue.authorizers[0].ref.set_account(vast::testing::tester::get_public_key("vast"));
    nf.manage.authorizers[0].ref.set_account(vast::testing::tester::get_public_key("vast"));
    auto auths      = std::vector<name>{N(vast)};

    auto am    = addmeta();
    am.creator = vast::testing::tester::get_public_key("vast");
...
BM_Action_newsuspend(benchmark::State& state) {
    ...
    ns.proposer = vast::testing::tester::get_public_key("vast");
    ...
    for(auto _ : state) {
        ...
        newdom.creator     = vast::testing::tester::get_public_key("vast");
        newdom.issue.authorizers[0].ref.set_account(vast::testing::tester::get_public_key("vast"));
        newdom.manage.authorizers[0].ref.set_account(vast::testing::tester::get_public_key("vast"));
        newdom.transfer.authorizers[0].ref.set_account(vast::testing::tester::get_public_key("vast"));
...
BM_Action_aprvsuspend(benchmark::State& state) {
    ...
    for(auto _ : state) {
        ...
        auto as       = aprvsuspend();
        as.name       = ns.name;
        auto sig      = vast::testing::tester::get_private_key("vast").sign(ns.trx.sig_digest(tester->control->get_chain_id()));
        as.signatures = {sig};
...
BM_Action_execsuspend(benchmark::State& state) {
    ...
    for(auto _ : state) {
        ...
        auto es     = execsuspend();
        es.name     = ns.name;
        es.executor = vast::testing::tester::get_public_key("vast");
