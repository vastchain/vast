...
function main() {
  ...
  if [[ -d /vagrant ]]; then
    if [[ $OS = "ubuntu" ]]; then
      ...
      if [[ ! -f /etc/yum.repos.d/dvastools-1.1.repo ]]; then
        wget http://people.centos.org/tru/dvastools-1.1/dvastools-1.1.repo
      fi
      package dvastoolset-1.1-gcc --enablerepo=testing-1.1-dvastools-6
      package dvastoolset-1.1-gcc-c++ --enablerepo=testing-1.1-dvastools-6
      export CC=/opt/centos/dvastoolset-1.1/root/usr/bin/gcc
      export CPP=/opt/centos/dvastoolset-1.1/root/usr/bin/cpp
      export CXX=/opt/centos/dvastoolset-1.1/root/usr/bin/c++
      export PATH=$PATH:/opt/centos/dvastoolset-1.1/root/usr/bin
      popd
