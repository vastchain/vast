...
parse_base58(const std::string& base58str) {
    constexpr auto prefix = config::public_key_vast_prefix;
...
public_key::operator std::string() const {
    ...
    return std::string(config::public_key_vast_prefix) + data_str;
}

std::ostream&
operator<<(std::ostream& s, const public_key& k) {
    s << "public_key(" << std::string(k) << ')';
    return s;
}

bool
operator==(const public_key& p1, const public_key& p2) {
...
using namespace std;
void
to_variant(const fc::crypto::public_key& var, fc::variant& vo) {
    vo = std::string(var);
}

void
from_variant(const fc::variant& var, fc::crypto::public_key& vo) {
