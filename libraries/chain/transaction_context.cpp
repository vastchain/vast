...
transaction_context::init(uint64_t initial_net_usage) {
    VAST_ASSERT(!is_initialized, transaction_exception, "cannot initialize twice");
    VAST_ASSERT(!trx.actions.empty(), tx_no_action, "There isn't any actions in this transaction");
    ...
    if(!control.charge_free_mode()) {
        ...
        check_paid();    // Fail early if there's no remaining available VAST & Pinned VAST tokens
    }
...
transaction_context::exec() {
    VAST_ASSERT(is_initialized, transaction_exception, "must first initialize");
...
transaction_context::check_time() const {
    ...
    if(BOOST_UNLIKELY(now > deadline)) {
        VAST_THROW(deadline_exception, "deadline exceeded", ("now", now)("deadline", deadline)("start", start));
    }
}
...
transaction_context::check_charge() {
    ...
    if(charge > trx.max_charge) {
        VAST_THROW(max_charge_exceeded_exception, "max charge exceeded, expected: ${ex}, max provided: ${mp}",
...
transaction_context::check_paid() const {
    ...
    case address::reserved_t: {
        VAST_THROW(payer_exception, "Reserved address cannot be used to be payer");
    }
    case address::public_key_t: {
        ...
        if(keys.find(payer.get_public_key()) == keys.end()) {
            VAST_THROW(payer_exception, "Payer ${p} needs to sign this transaction. keys: ${keys}", ("p",payer)("keys",keys));
        }
        break;
    }
    case address::generated_t: {
        ...
        case N(.domain): {
            for(auto& act : trx.actions) {
                VAST_ASSERT(act.domain == key, payer_exception, "Only actions in '${d}' domain can be paid by the payer", ("d",act.domain));
            }
            break;
        }
        case N(.fungible): {
            VAST_ASSERT(key != N128(VAST_SYM_ID) && key != N128(PVAST_SYM_ID), payer_exception, "VAST or PVAST is not allowed to use this payer");
            for(auto& act : trx.actions) {
                VAST_ASSERT(act.domain == N128(.fungible) && act.key == key, payer_exception, "Only actions with S#${d} fungible can be paid by the payer", ("d",act.key));
            }
            break;
        }
        default: {
            VAST_THROW(payer_exception, "Only domain or fungible generated address can be payer");
            break;
        }
        }  // switch
        break;
    ...
    property vast, pvast;
    READ_DB_ASSET_NO_THROW(payer, PVAST_SYM_ID, pvast);
    if(pvast.amount > charge) {
        return;
    }

    READ_DB_ASSET_NO_THROW(payer, VAST_SYM_ID, vast);
    if(pvast.amount + vast.amount >= charge) {
        return;
    }
    VAST_THROW(charge_exceeded_exception, "There are only ${e} and ${p} left, but charge is ${c}",
        ("e",asset(vast.amount, vast_sym()))("p",asset(pvast.amount, pvast_sym()))("c",asset(charge,vast_sym())));
}
...
transaction_context::check_net_usage() const {
    if(!control.skip_trx_checks()) {
        VAST_ASSERT(net_usage < net_limit, tx_net_usage_exceeded, "transaction net usage is too high: ${net_usage} > ${net_limit}",
...
transaction_context::record_transaction(const transaction_id_type& id, fc::time_point_sec expire) {
    try {
        ...
        VAST_ASSERT(false, tx_duplicate,
