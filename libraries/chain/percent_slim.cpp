...
percent_slim::from_string(const string& from) {
    ...
    try {
        ...
        if(dot_pos != string::npos) {
            VAST_ASSERT((dot_pos != s.size() - 1), percent_type_exception,
        ...
        if(dot_pos != string::npos) {
            p = s.size() - dot_pos - 1;
            VAST_ASSERT2(p <= kPrecision, percent_type_exception,
    ...
    VAST_CAPTURE_AND_RETHROW(percent_type_exception, (from));
}
