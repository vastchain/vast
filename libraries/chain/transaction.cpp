...
transaction::get_signature_keys(const signatures_base_type& signatures, const chain_id_type& chain_id,
    ...
    try {
        ...
        for(auto& sig : signatures) {
            ...
            VAST_ASSERT(allow_duplicate_keys || successful_insertion, tx_duplicate_sig,
...
packed_transaction::reflector_init() {
    ...
    VAST_ASSERT(unpacked_trx.expiration == time_point_sec(), tx_decompression_error, "packed_transaction already unpacked");
...
packed_transaction::get_unprunable_size() const {
    ...
    VAST_ASSERT(size <= std::numeric_limits<uint32_t>::max(), tx_too_big, "packed_transaction is too big");
...
struct read_limiter {
    ...
    write(Sink& sink, const char* s, size_t count) {
        VAST_ASSERT(_total + count <= Limit, tx_decompression_error, "Exceeded maximum decompressed transaction size");
        _total += count;
...
packed_transaction::local_unpack_transaction() {
    try {
        ...
        case zlib:
            ...
            break;
        default:
            VAST_THROW(unknown_transaction_compression, "Unknown transaction compression algorithm");
        }
    }
