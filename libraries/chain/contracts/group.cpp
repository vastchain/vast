...
group::visit_root(const visit_func& func) const {
    VAST_ASSERT(nodes_.size() > 0, group_type_exception, "There's not any node defined in this group");
...
void
to_variant(const vast::chain::contracts::group& group, fc::variant& v) {
    fc::mutable_variant_object mv;
...
from_variant(const fc::variant& v, group& group, group::node& node, uint32_t depth) {
    VAST_ASSERT(depth < config::default_max_auth_depth, group_type_exception, "Exceeds max node depth");
    ...
    if(vo.find("threshold") == vo.end()) {
        ...
        VAST_ASSERT(group.keys_.size() < std::numeric_limits<decltype(node.index)>::max(), group_type_exception, "Exceeds max keys limit");
    ...
    VAST_ASSERT(cvs.size() < std::numeric_limits<decltype(node.size)>::max(), group_type_exception, "Exceeds max child nodes limit");
    VAST_ASSERT(group.nodes_.size() + cvs.size() < std::numeric_limits<decltype(node.index)>::max(), group_type_exception, "Exceeds max nodes limit");
...
void
from_variant(const fc::variant& v, vast::chain::contracts::group& group) {
    auto& vo = v.get_object();
    if(vo.find("name") != vo.end()) {
        from_variant(vo["name"], group.name_);
    }
    VAST_ASSERT(!group.key_.is_generated(), group_type_exception, "Generated group key is not allowed here");
