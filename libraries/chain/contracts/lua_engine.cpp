...
#include <vast/chain/contracts/lua_db.hpp>
#include <vast/chain/contracts/lua_json.hpp>
...
requirex(lua_State* L) {
    ...
    VAST_ASSERT2(r == LUA_OK, script_load_exceptoin, "Load module '{}' script failed: {}", module, lua_tostring(L, -1));
...
setup_luastate(token_database_cache& tokendb_cache, int checks) {
    ...
    auto r = luaL_loadstring(L, script->content.c_str());
    VAST_ASSERT2(r == LUA_OK, script_load_exceptoin, "Load loader script failed: {}", lua_tostring(L, -1));
...
lua_engine::invoke_filter(const controller& control, const action& act, const script_name& script) {
    ...
    VAST_ASSERT2(r == LUA_OK, script_load_exceptoin, "Load '{}' script failed: {}", script, lua_tostring(L, -1));
    ...
    VAST_ASSERT2(r2 == LUA_OK, script_execution_exceptoin, "Convert action to json failed: {}", lua_tostring(L, -1));
    ...
    VAST_ASSERT2(r3 == LUA_OK, script_execution_exceptoin, "Lua script executed failed: {}", lua_tostring(L, -1));

    VAST_ASSERT2(lua_gettop(L) >= 2, script_invalid_result_exceptoin, "No result is returned from script, should at least be one");
    VAST_ASSERT2(lua_isboolean(L, -1), script_invalid_result_exceptoin, "Result returned from lua filter should be boolean value");
