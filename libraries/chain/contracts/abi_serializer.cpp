...
abi_serializer::configure_built_in_types() {
    ...
    built_in_types_.emplace("vast_link", pack_unpack<vast_link>());
...
abi_serializer::set_abi(const abi_def& abi) {
    ...
    for(const auto& td : abi.types) {
        VAST_ASSERT(_is_type(td.type), invalid_type_inside_abi, "invalid type ${type}", ("type", td.type));
        VAST_ASSERT(!_is_type(td.new_type_name), duplicate_abi_type_def_exception, "type already exists", ("new_type_name", td.new_type_name));
        typedefs_[td.new_type_name] = td.type;
    }

    VAST_ASSERT(typedefs_.size() == abi.types.size(), duplicate_abi_type_def_exception, "duplicate type definition detected");
    VAST_ASSERT(structs_.size() == abi.structs.size(), duplicate_abi_struct_def_exception, "duplicate struct definition detected");
    VAST_ASSERT(variants_.size() == abi.variants.size(), duplicate_abi_variant_def_exception, "duplicate variant definition detected");
    VAST_ASSERT(enums_.size() == abi.enums.size(), duplicate_abi_enum_def_exception, "duplicate enum definition detected");
...
abi_serializer::get_integer_size(const type_name& type) const {
    auto stype = type;
    VAST_ASSERT(is_integer(type), invalid_type_inside_abi, "${stype} is not an integer type", ("stype", stype));
...
abi_serializer::get_struct(const type_name& type) const {
    ...
    VAST_ASSERT(itr != structs_.end(), invalid_type_inside_abi, "Unknown struct ${type}", ("type", type));
    return itr->second;
}
...
abi_serializer::validate() const {
    for(const auto& t : typedefs_) {
        try {
            ...
            while(itr != typedefs_.end()) {
                VAST_ASSERT(find(types_seen.begin(), types_seen.end(), itr->second) == types_seen.end(), abi_circular_def_exception, "Circular reference in type ${type}", ("type", t.first));
    ...
    for(const auto& t : typedefs_) {
        try {
            VAST_ASSERT(_is_type(t.second), invalid_type_inside_abi, "${type}", ("type", t.second));
        }
        FC_CAPTURE_AND_RETHROW((t))
    }
        ...
        try {
            if(s.second.base != type_name()) {
                ...
                while(current.base != type_name()) {
                    ...
                    VAST_ASSERT(find(types_seen.begin(), types_seen.end(), base.name) == types_seen.end(), abi_circular_def_exception, "Circular reference in struct ${type}", ("type", s.second.name));
            ...
            for(const auto& field : s.second.fields) {
                try {
                    VAST_ASSERT(_is_type(field.type), invalid_type_inside_abi, "${type}", ("type", field.type));
                }
    ...
    for(const auto& et : enums_) {
        try {
            VAST_ASSERT(_is_type(et.second.integer), invalid_type_inside_abi, "${type}", ("type", et.second.integer));
        }
...
abi_serializer::_binary_to_variant(const type_name& type, fc::datastream<const char*>& stream,
    ...
    VAST_ASSERT(s_itr != structs_.end(), invalid_type_inside_abi, "Unknown type ${type}", ("type", ctx.maybe_shorten(type)));
    ...
    for(auto i = 0u; i < st.fields.size(); ++i) {
        ...
        if(!stream.remaining()) {
            VAST_THROW(unpack_exception, "Stream unexpectedly ended; unable to unpack field '${f}' of struct '${p}'",
...
abi_serializer::_binary_to_variant(const type_name& type, fc::datastream<const char*>& stream,
    ...
    if(btype != built_in_types_.end()) {
        ...
        VAST_RETHROW_EXCEPTIONS(unpack_exception, "Unable to unpack ${class} type '${type}' while processing '${p}'",
    ...
    if(is_array(rtype)) {
        ...
        VAST_RETHROW_EXCEPTIONS(unpack_exception, "Unable to unpack size of array '${p}'", ("p", ctx.get_path_string()))
        ...
        for(decltype(size.value) i = 0; i < size; ++i) {
            ...
            VAST_ASSERT(!v.is_null(), unpack_exception, "Invalid packed array '${p}'", ("p", ctx.get_path_string()));
            vars.emplace_back(std::move(v));
        }
        // QUESTION: Why would the assert below ever fail?
        VAST_ASSERT(vars.size() == size.value, unpack_exception,
        ...
        VAST_RETHROW_EXCEPTIONS(unpack_exception, "Unable to unpack presence flag of optional '${p}'", ("p", ctx.get_path_string()))
        ...
        VAST_RETHROW_EXCEPTIONS(unpack_exception, "Unable to unpack index of variant '${p}'", ("p", ctx.get_path_string()));

        auto& vt = v_itr->second;
        VAST_ASSERT2((uint32_t)i < vt.fields.size(), unpack_exception, "Index of variant '{}' if not valid", ctx.get_path_string());
        ...
        VAST_ASSERT2(ev.as_uint64() < et.fields.size(), unpack_exception, "Value of enum '{}' is not valid", ctx.get_path_string());
...
abi_serializer::_binary_to_variant(const type_name& type, const bytes& binary, impl::binary_to_variant_context& ctx) const {
    ...
    if(ds.remaining() > 0) {
        VAST_THROW2(unpack_exception, "Binary buffer is not EOF after unpack variable, remaining: {} bytes.", ds.remaining());
    }
    return var;
}
    ...
    try {
        ...
        if(btype != built_in_types_.end()) {
            ...
            auto check_field = [&](auto& vo, auto name, std::string type) {
                VAST_ASSERT2(vo.contains(name), pack_exception,
                    "Missing field '{}' in input object while processing variant '{}'", name, ctx.get_path_string());
                if(type == "string") {
                    VAST_ASSERT2(vo[name].is_string(), pack_exception,
            ...
            VAST_ASSERT2(index < vt.fields.size(), pack_exception, "Invalid 'type' value of variant '{}'", ctx.get_path_string());
            ...
            VAST_ASSERT2(index < et.fields.size(), pack_exception, "Invalid value of enum '{}'", ctx.get_path_string());
            ...
            if(var.is_object()) {
                ...
                for(uint32_t i = 0; i < st.fields.size(); ++i) {
                    ...
                    if(vo.contains(string(field.name).c_str())) {
                        ...
                        VAST_THROW(pack_exception, "Missing field '${f}' in input object while processing struct '${p}'",
                                  ("f", ctx.maybe_shorten(field.name))("p", ctx.get_path_string()));
                    }
                }
            }
                ...
                VAST_ASSERT(st.base == type_name(), invalid_type_inside_abi,
                ...
                for(uint32_t i = 0; i < st.fields.size(); ++i) {
                    ...
                    if(va.size() > i) {
                        ...
                        VAST_THROW(pack_exception, "Early end to input array specifying the fields of struct '${p}'; require input for field '${f}'",
                                  ("p", ctx.get_path_string())("f", ctx.maybe_shorten(field.name)));
                    }
                }
            }
            else {
                VAST_THROW(pack_exception, "Unexpected input encountered while processing struct '${p}'", ("p", ctx.get_path_string()));
            }
        }
        else {
            VAST_THROW(invalid_type_inside_abi, "Unknown type ${type}", ("type", ctx.maybe_shorten(type)));
        }
    }
...
abi_serializer::_variant_to_binary(const type_name& type, const fc::variant& var, impl::variant_to_binary_context& ctx) const {
    try {
        auto h = ctx.enter_scope();
        VAST_ASSERT2(_is_type(type), unknown_abi_type_exception, "Unknown type: {} in ABI", type);
...
abi_traverse_context::check_deadline() const {
    VAST_ASSERT(std::chrono::steady_clock::now() < deadline, abi_serialization_deadline_exception,
...
abi_traverse_context::enter_scope() {
    ...
    ++recursion_depth;
    VAST_ASSERT(recursion_depth < abi_serializer::max_recursion_depth, abi_recursion_depth_exception,
...
abi_traverse_context_with_path::push_to_path(const path_item& item) {
    std::function<void()> callback = [this]() {
        VAST_ASSERT(path.size() > 0, abi_exception,
...
abi_traverse_context_with_path::set_array_index_of_path_back(uint32_t i) {
    VAST_ASSERT(path.size() > 0, abi_exception, "path is empty");

    auto& b = path.back();

    VAST_ASSERT(b.contains<array_index_path_item>(), abi_exception, "trying to set array index without first pushing new array index item");
