...
authorizer_ref::to_string() const {
    ...
    default: {
        VAST_ASSERT(false, authorizer_ref_type_exception, "Not valid ref type: ${type}", ("type",this->type()));
    }
    }  // switch
}
...
using vast::chain::contracts::authorizer_ref;
...
from_variant(const fc::variant& v, authorizer_ref& ref) {
    ...
    VAST_ASSERT(str.size() > 4, authorizer_ref_type_exception, "Not valid authorizer ref string");
    ...
    VAST_ASSERT(false, authorizer_ref_type_exception, "Unknown authorizer ref prefix: ${prefix}", ("prefix",str.substr(0,4)));
}
