...
const char* URI_SCHEMA  = "https://vast.li/";
const char* URI_SCHEMA2 = "vastlink://";
...
decode(const std::string& nums, uint pos, uint end) {
    ...
    VAST_ASSERT(pz != string::npos, vast_link_exception, "Invalid VAST-Link");

    for(auto i = pz; i < end; i++) {
        auto c = strchr(ALPHABETS, nums[i]);
        FC_ASSERT(c != nullptr, "invalid character in vast-link");
        num *= 42;
        num += (c - ALPHABETS);
    }
...
parse_segments(const bytes& b, uint16_t& header) {
    ...
    auto segs = vast_link::segments_type();

    auto i  = 2u;
    auto pk = 0u;
    while(i < b.size()) {
        auto k = (uint8_t)b[i];
        VAST_ASSERT(k > pk, vast_link_exception, "Segments are not ordered by keys");
        pk = k;

        if(k <= 20) {
            FC_ASSERT(b.size() > i + 1); // value is 1 byte
            auto v = (uint8_t)b[i + 1];
            segs.emplace(k, vast_link::segment(k, v));
            ...
            segs.emplace(k, vast_link::segment(k, boost::endian::big_to_native(v)));
            ...
            if(sz > 0) {
                ...
                segs.emplace(k, vast_link::segment(k, std::string(&b[i] + 1 + s, sz)));
            }
            else {
                segs.emplace(k, vast_link::segment(k, std::string()));
            }

            i += 1 + s + sz;
        }
        else {
            VAST_THROW(vast_link_exception, "Invalid key type: ${k}", ("k",k));
        }
    }
    return segs;
}

vast_link::signatures_type
parse_signatures(const bytes& b) {
    FC_ASSERT(b.size() > 0 && b.size() % 65 == 0);
    auto sigs = vast_link::signatures_type();
...
vast_link
vast_link::parse_from_vastli(const std::string& str) {
    using namespace internal;

    VAST_ASSERT(str.size() < 400, vast_link_exception, "Link is too long, max length allowed: 400");
    VAST_ASSERT(str.size() > 20, vast_link_exception, "Link is too short");
...
const vast_link::segment&
vast_link::get_segment(uint8_t key) const {
    auto it = segments_.find(key);
    VAST_ASSERT(it != segments_.end(), vast_link_no_key_exception, "Cannot find segment for key: ${k}", ("k",key));

    return it->second;
}

bool
vast_link::has_segment(uint8_t key) const {
    return segments_.find(key) != segments_.end();
}

link_id_type
vast_link::get_link_id() const {
    auto& seg = get_segment(link_id);
    VAST_ASSERT(seg.strv && seg.strv->size() == sizeof(link_id_type), vast_link_id_exception, "Not valid link id in this VAST-Link");
...
void
write_segments_bytes(const vast_link& link, Stream& stream) {
...
template<typename Stream>
void
write_signatures_bytes(const vast_link& link, Stream& stream) {
...
std::string
vast_link::to_string(int prefix) const {
    using namespace internal;
...
void
vast_link::add_segment(const segment& seg) {
...
void
vast_link::remove_segment(uint8_t key) {
    segments_.erase(key);
}

void
vast_link::add_signature(const signature_type& sig) {
    signatures_.emplace(sig);
}

void
vast_link::sign(const private_key_type& pkey) {
...
void
from_variant(const fc::variant& v, vast_link& link) {
    link = vast_link::parse_from_vastli(v.get_string());
}

void
to_variant(const vast::chain::contracts::vast_link& link, fc::variant& v) {
