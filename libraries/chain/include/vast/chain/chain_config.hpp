...
struct chain_config {
    ...
    uint32_t vast_link_expired_secs;
    ...
    operator<<(Stream& out, const chain_config& c) {
        return out << "Max Block Net Usage: " << c.max_block_net_usage << ", "
                   ...
                   << "VAST-Link expried secs: " << c.vast_link_expired_secs << "\n";
    }
...
FC_REFLECT(vast::chain::chain_config,
          ...
          (max_transaction_lifetime)(max_authority_depth)(vast_link_expired_secs)
          );
