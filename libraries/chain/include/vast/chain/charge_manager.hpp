...
public:
    charge_manager(const controller& control, const vast_execution_context& exec_ctx)
        : control_(control)
...
public:
    ...
    calculate(const packed_transaction& ptrx, size_t sig_num = 0) const {
        ...
        VAST_ASSERT(!trx.actions.empty(), tx_no_action, "There's not any actions in this transaction");
...
struct act_charge<N(issuefungible), T> : public base_act_charge {
    ...
    extra_factor(const action& act) {
        ...
        // set charge to zero when issuing VAST
        if(sym == vast_sym()) {
            return 0;
        }
        return 1;
