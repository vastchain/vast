...
#include <vast/utilities/common.hpp>
...
class variant;
void to_variant(const vast::chain::address& addr, fc::variant& v);
void from_variant(const fc::variant& v, vast::chain::address& addr);

template<>
inline void
to_variant<vast::chain::address>(const vast::chain::address& addr, fc::variant& v) {
    to_variant(addr, v);
}

template<>
inline void
from_variant<vast::chain::address>(const fc::variant& v, vast::chain::address& addr) {
    from_variant(v, addr);
}
...
template <>
struct formatter<vast::chain::address> {
    template <typename ParseContext>
    constexpr auto parse(ParseContext& ctx) { return ctx.begin(); }

    template <typename FormatContext>
    auto format(const vast::chain::address& addr, FormatContext &ctx) {
...
FC_REFLECT(vast::chain::address, (storage_))
