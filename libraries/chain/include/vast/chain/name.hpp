...
#define N(X) vast::chain::string_to_name(#X)
...
namespace std {
template <>
struct hash<vast::chain::name> : private hash<uint64_t> {
    using argument_type = vast::chain::name;
...
class variant;
void to_variant(const vast::chain::name& name, fc::variant& v);
void from_variant(const fc::variant& v, vast::chain::name& name);
...
template <>
struct formatter<vast::chain::name> {
    template <typename ParseContext>
    constexpr auto parse(ParseContext& ctx) { return ctx.begin(); }

    template <typename FormatContext>
    auto format(const vast::chain::name& n, FormatContext &ctx) {
...
FC_REFLECT(vast::chain::name, (value));
