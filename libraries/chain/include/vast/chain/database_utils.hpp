...
template<typename T>
void
to_variant(const vast::chain::shared_vector<T>& sv, variant& v) {
    to_variant(std::vector<T>(sv.begin(), sv.end()), v);
}

template<typename T>
void from_variant(const variant& v, vast::chain::shared_vector<T>& sv) {
    std::vector<T> _v;
    from_variant(v, _v);
    sv = vast::chain::shared_vector<T>(_v.begin(), _v.end(), sv.get_allocator());
}
