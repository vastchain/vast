...
namespace vast { namespace chain { namespace config {
...
const static uint128_t system_account_name = N128(vast);
...
const static int default_vast_link_expired_secs = 20;  // 20s -> total: 40s
...
}}}  // namespace vast::chain::config

template <typename Number>
Number
VAST_PERCENT(Number value, int percentage) {
    return value * percentage / vast::chain::config::percent_100;
}

template <typename Number>
Number
VAST_PERCENT_CEIL(Number value, uint32_t percentage) {
    return ((value * percentage) + vast::chain::config::percent_100 - vast::chain::config::percent_1)
           / vast::chain::config::percent_100;
}
