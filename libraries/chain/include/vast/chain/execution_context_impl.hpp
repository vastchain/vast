...
public:
    ...
    index_of(name act) const override {
        ...
        VAST_ASSERT(it != std::cend(arr) && *it == act.value, unknown_action_exception, "Unknown action: ${act}", ("act", act));
    ...
    set_version(name act, int newver) override {
        ...
        VAST_ASSERT2(newver > cver && newver <= (int)mver, action_version_exception, "New version should be in range ({},{}]", cver, mver);
    ...
    invoke(int actindex, Args&&... args) const {
        ...
        VAST_ASSERT(result.has_value(), action_index_exception, "Invalid action index: ${act}", ("act", actindex));
        if constexpr (!std::is_void<RType>::value) {
            return *result;
        }
    }
...
using vast_execution_context = execution_context_impl<
                                  contracts::newdomain,
