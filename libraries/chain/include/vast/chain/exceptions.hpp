...
#define VAST_ASSERT(expr, exc_type, FORMAT, ...)            \
    FC_MULTILINE_MACRO_BEGIN                               \
    if(!(expr))                                            \
        FC_THROW_EXCEPTION(exc_type, FORMAT, __VA_ARGS__); \
    FC_MULTILINE_MACRO_END

#define VAST_ASSERT2(expr, exc_type, FORMAT, ...)              \
    FC_MULTILINE_MACRO_BEGIN                                  \
    if(!(expr))                                               \
        FC_THROW_EXCEPTION2(exc_type, FORMAT, ##__VA_ARGS__); \
    FC_MULTILINE_MACRO_END

#define VAST_THROW(exc_type, FORMAT, ...)  throw exc_type(FC_LOG_MESSAGE(error, FORMAT, __VA_ARGS__));
#define VAST_THROW2(exc_type, FORMAT, ...) throw exc_type(FC_LOG_MESSAGE2(error, FORMAT, ##__VA_ARGS__));
 ...
 */
#define VAST_RETHROW_EXCEPTIONS(exception_type, FORMAT, ...)                                                 \
...
#define VAST_RETHROW_EXCEPTIONS2(exception_type, FORMAT, ...)                                                 \
    catch(const boost::interprocess::bad_alloc&) {                                                           \
        throw;                                                                                               \
    }                                                                                                        \
    catch(const fc::unrecoverable_exception&) {                                                              \
 ...
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define VAST_CAPTURE_AND_RETHROW(exception_type, ...)                                                               \
    catch(const boost::interprocess::bad_alloc&) {                                                                 \
        throw;                                                                                                     \
    }                                                                                                              \
    catch(const fc::unrecoverable_exception&) {                                                                    \
...
#define VAST_RECODE_EXC(cause_type, effect_type)    \
...
FC_DECLARE_DERIVED_EXCEPTION( charge_exceeded_exception,       transaction_exception, 3030011, "exceeded remaining VAST & Pinned VAST tokens" );
...
FC_DECLARE_DERIVED_EXCEPTION( vast_link_exception,            action_exception,   3040700, "VAST-Link exception" );
FC_DECLARE_DERIVED_EXCEPTION( vast_link_no_key_exception,     vast_link_exception, 3040701, "Specific segment key is not in this vast-link." );
FC_DECLARE_DERIVED_EXCEPTION( vast_link_version_exception,    vast_link_exception, 3040702, "VAST-Link version is not valid.");
FC_DECLARE_DERIVED_EXCEPTION( vast_link_id_exception,         vast_link_exception, 3040703, "VAST-Link id is not valid.");
FC_DECLARE_DERIVED_EXCEPTION( vast_link_dupe_exception,       vast_link_exception, 3040704, "Duplicate VAST-Link.");
FC_DECLARE_DERIVED_EXCEPTION( vast_link_type_exception,       vast_link_exception, 3040705, "Invalid VAST-Link type.");
FC_DECLARE_DERIVED_EXCEPTION( vast_link_expiration_exception, vast_link_exception, 3040706, "VAST-Link is expired.");
FC_DECLARE_DERIVED_EXCEPTION( vast_link_existed_exception,    vast_link_exception, 3040707, "VAST-Link is not existed.");
FC_DECLARE_DERIVED_EXCEPTION( everipass_exception,           vast_link_exception, 3040708, "everiPass failed.");
FC_DECLARE_DERIVED_EXCEPTION( everipay_exception,            vast_link_exception, 3040709, "everiPay failed.");
...
FC_DECLARE_DERIVED_EXCEPTION( missing_vast_api_plugin_exception,      plugin_exception, 3130004, "Missing VAST API Plugin" );
...
FC_DECLARE_DERIVED_EXCEPTION( vast_link_plugin_exception,            chain_exception, 3190000, "vast-link plugin exception" );
FC_DECLARE_DERIVED_EXCEPTION( vast_link_not_existed_now_excetpion,   vast_link_plugin_exception, 3190001, "VAST-Link is not existed currently" );
FC_DECLARE_DERIVED_EXCEPTION( vast_link_already_watched_exception,   vast_link_plugin_exception, 3190002, "VAST-Link is already watched" );
FC_DECLARE_DERIVED_EXCEPTION( exceed_vast_link_watch_time_exception, vast_link_plugin_exception, 3190003, "Exceed VAST-Link watch time" );
