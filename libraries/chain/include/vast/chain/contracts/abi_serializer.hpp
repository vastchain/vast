...
struct abi_from_variant {
    ...
    extract(const variant& v, action& act, abi_traverse_context& ctx) {
        ...
        VAST_ASSERT(vo.contains("name"), action_type_exception, "Missing name");
        VAST_ASSERT(vo.contains("domain"), action_type_exception, "Missing domain");
        VAST_ASSERT(vo.contains("key"), action_type_exception, "Missing key");
        ...
        VAST_ASSERT(valid_empty_data || !act.data.empty(), packed_transaction_type_exception,
    ...
    extract(const variant& v, packed_transaction& ptrx, abi_traverse_context& ctx) {
        auto        h  = ctx.enter_scope();
        const auto& vo = v.get_object();
        VAST_ASSERT(vo.contains("signatures"), packed_transaction_type_exception, "Missing signatures");
        VAST_ASSERT(vo.contains("compression"), packed_transaction_type_exception, "Missing compression");
        ...
        if(vo.contains("packed_trx") && vo["packed_trx"].is_string() && !vo["packed_trx"].as_string().empty()) {
            ...
            VAST_ASSERT(vo.contains("transaction"), packed_transaction_type_exception, "Missing transaction");
