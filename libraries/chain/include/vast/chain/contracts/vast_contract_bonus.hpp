...
auto check_n_rtn = [](auto& asset, auto sym, auto ctype) -> decltype(asset) {
    VAST_ASSERT2(asset.sym() == sym, bonus_asset_exception, "Invalid symbol of assets, expected: {}, provided:  {}", sym, asset.sym());
    switch(ctype) {
    case bonus_check_type::natural: {
        VAST_ASSERT2(asset.amount() >= 0, bonus_asset_exception, "Invalid amount of assets, must be natural number. Provided: {}", asset);
        break;
    }
    case bonus_check_type::positive: {
        VAST_ASSERT2(asset.amount() > 0, bonus_asset_exception, "Invalid amount of assets, must be positive. Provided: {}", asset);
        break;
    }
    }  // switch
...
check_bonus_receiver(const token_database& tokendb, const dist_receiver& receiver) {
    ...
    case dist_receiver_type::address: {
        ...
        VAST_ASSERT2(addr.is_public_key(), bonus_receiver_exception, "Only public key address can be used for receiving bonus now.");
        break;
    }
    case dist_receiver_type::ftholders: {
        ...
        VAST_ASSERT2(tokendb.exists_token(token_type::fungible, std::nullopt, sym_id),
...
check_bonus_rules(const token_database& tokendb, const T& rules, asset amount) {
    ...
    for(auto& rule : rules) {
        ...
        case dist_rule_type::fixed: {
            VAST_ASSERT2(remain_percent == 0, bonus_rules_order_exception,
            ...
            VAST_ASSERT2(frv.amount() <= remain, bonus_rules_exception,
        ...
        case dist_rule_type::percent: {
            ...
            // check valid precent
            VAST_ASSERT2(p > 0 && p <= 1, bonus_percent_value_exception,
            ...
            // check large than remain
            VAST_ASSERT2(prv <= remain, bonus_rules_exception,
            ...
            VAST_ASSERT2(prv >= 1, bonus_percent_result_exception,
        ...
        case dist_rule_type::remaining_percent: {
            VAST_ASSERT2(remain > 0, bonus_rules_exception, "There's no bonus left for reamining-percent rule to distribute");
            ...
            auto p = (percent_type)pr.percent;
            // check valid precent
            VAST_ASSERT2(p > 0 && p <= 1, bonus_percent_value_exception, "Precent value should be in range (0,1]");
            auto prv = (int64_t)boost::multiprecision::floor(p * real_type(remain));
            // check percent result is large than minial unit of asset
            VAST_ASSERT2(prv >= 1, bonus_percent_result_exception,
                "Rule #{} is not valid, the amount for this rule shoule be as least large than one unit of asset, but it's zero now.", index);
            remain_percent += p;
            VAST_ASSERT2(remain_percent <= 1, bonus_percent_value_exception, "Sum of remaining percents is large than 100%, current: {}", get_percent_string(remain_percent));
            break;
        }
        }  // switch
        index++;
    ...
    if(remain > 0) {
        VAST_ASSERT2(remain_percent == 1, bonus_rules_not_fullfill,
...
check_passive_methods(const execution_context& exec_ctx, const passive_methods& methods) {
    for(auto& it : methods) {
        ...
        VAST_ASSERT2(it.action == N(transferft) || it.action == N(everipay), bonus_method_exception,
...
VAST_ACTION_IMPL_BEGIN(setpsvbonus) {
    ...
    try {
        ...
        if constexpr (VAST_ACTION_VER() == 1) {
            ...
            VAST_ASSERT(context.has_authorized(N128(.bonus), name128::from_number(sym_id)), action_authorize_exception,
                "Invalid authorization fields in action(domain and key).");
        }
        else {
            sym_id = spbact.sym_id;
            VAST_ASSERT(context.has_authorized(N128(.psvbonus), name128::from_number(spbact.sym_id)), action_authorize_exception,
                "Invalid authorization fields in action(domain and key).");
        }
        ...
        if constexpr (VAST_ACTION_VER() == 1) {
            VAST_ASSERT2(fungible->sym == spbact.sym, bonus_symbol_exception, "Symbol provided is not the same as FT");
        }
        auto sym = fungible->sym;

        VAST_ASSERT(sym != vast_sym(), bonus_symbol_exception, "Passive bonus cannot be registered in VAST");
        VAST_ASSERT(sym != pvast_sym(), bonus_symbol_exception, "Passive bonus cannot be registered in Pinned VAST");

        VAST_ASSERT2(!tokendb.exists_token(token_type::psvbonus, std::nullopt, get_psvbonus_db_key(sym.id(), kPsvBonus)),
            bonus_dupe_exception, "It's now allowd to update passive bonus currently.");

        auto rate = (percent_type)spbact.rate;
        VAST_ASSERT2(rate > 0 && rate <= 1, bonus_percent_value_exception,
        ...
        if(spbact.minimum_charge.has_value()) {
            ...
            if(spbact.charge_threshold.has_value()) {
                VAST_ASSERT2(*spbact.minimum_charge < *spbact.charge_threshold, bonus_rules_exception,
        ...
        VAST_ASSERT2(spbact.rules.size() > 0, bonus_rules_exception, "Rules for passive bonus cannot be empty");
...
VAST_ACTION_IMPL_BEGIN(distpsvbonus) {
    using namespace internal;
    ...
    try {
        ...
        VAST_ASSERT2(pbonus.amount >= pb->dist_threshold.amount(), bonus_unreached_dist_threshold,
...
FC_REFLECT(vast::chain::contracts::internal::holder_dist, (sym_id)(slim)(coll)(total));
FC_REFLECT(vast::chain::contracts::internal::bonusdist, (created_at)(created_index)(holders)(deadline)(final_receiver));
