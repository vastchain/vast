...
VAST_ACTION_IMPL_BEGIN(newdomain) {
    using namespace internal;

    auto ndact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(ndact.name, N128(.create)), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        check_name_reserved(ndact.name);

        DECLARE_TOKEN_DB()
        VAST_ASSERT(!tokendb.exists_token(token_type::domain, std::nullopt, ndact.name), domain_duplicate_exception,
            "Domain ${name} already exists.", ("name",ndact.name));

        VAST_ASSERT(ndact.issue.name == "issue", permission_type_exception,
            "Name ${name} does not match with the name of issue permission.", ("name",ndact.issue.name));
        VAST_ASSERT(ndact.issue.threshold > 0 && validate(ndact.issue), permission_type_exception,
            "Issue permission is not valid, which may be caused by invalid threshold, duplicated keys.");
        VAST_ASSERT(ndact.transfer.name == "transfer", permission_type_exception,
            "Name ${name} does not match with the name of transfer permission.", ("name",ndact.transfer.name));
        VAST_ASSERT(validate(ndact.transfer), permission_type_exception,
        ...
        VAST_ASSERT(ndact.manage.name == "manage", permission_type_exception,
            "Name ${name} does not match with the name of manage permission.", ("name",ndact.manage.name));
        VAST_ASSERT(validate(ndact.manage), permission_type_exception,
...
VAST_ACTION_IMPL_BEGIN(issuetoken) {
    using namespace internal;

    auto itact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(itact.domain, N128(.issue)), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
        VAST_ASSERT(!itact.owner.empty(), token_owner_exception, "Owner cannot be empty.");
        for(auto& o : itact.owner) {
            check_address_reserved(o);
        }

        DECLARE_TOKEN_DB()
        VAST_ASSERT2(tokendb.exists_token(token_type::domain, std::nullopt, itact.domain), unknown_domain_exception,
            "Cannot find domain: {}.", itact.domain);

        auto check_name = [&](const auto& name) {
            check_name_reserved(name);
            VAST_ASSERT2(!tokendb.exists_token(token_type::token, itact.domain, name), token_duplicate_exception,
...
VAST_ACTION_IMPL_BEGIN(transfer) {
    using namespace internal;

    auto ttact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(ttact.domain, ttact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
        VAST_ASSERT(!ttact.to.empty(), token_owner_exception, "New owner cannot be empty.");
        ...
        VAST_ASSERT(!check_token_destroy(*token), token_destroyed_exception, "Destroyed token cannot be transfered.");
        VAST_ASSERT(!check_token_locked(*token), token_locked_exception, "Locked token cannot be transfered.");
...
VAST_ACTION_IMPL_BEGIN(destroytoken) {
    using namespace internal;

    auto dtact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(dtact.domain, dtact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
        ...
        if(dd.has_value() && *dd == "true") {
            VAST_THROW(token_cannot_destroy_exception, "Token in this domain: ${d} cannot be destroyed", ("d",dtact.domain));
        }
        ...
        VAST_ASSERT(!check_token_destroy(*token), token_destroyed_exception, "Token is already destroyed.");
        VAST_ASSERT(!check_token_locked(*token), token_locked_exception, "Locked token cannot be destroyed.");
...
VAST_ACTION_IMPL_BEGIN(updatedomain) {
    using namespace internal;

    auto udact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(udact.name, N128(.update)), action_authorize_exception,
        ...
        if(udact.issue.has_value()) {
            VAST_ASSERT(udact.issue->name == "issue", permission_type_exception,
                "Name ${name} does not match with the name of issue permission.", ("name",udact.issue->name));
            VAST_ASSERT(validate(*udact.issue), permission_type_exception,
                "Issue permission is not valid, which may be caused by invalid threshold, duplicated keys.");
        ...
        if(udact.transfer.has_value()) {
            ...
            if(dt.has_value() && *dt == "true") {
                VAST_THROW(domain_cannot_update_exception, "Transfer permission of this domain cannot be updated");
            }

            VAST_ASSERT(udact.transfer->name == "transfer", permission_type_exception,
                "Name ${name} does not match with the name of transfer permission.", ("name",udact.transfer->name));
            VAST_ASSERT(validate(*udact.transfer), permission_type_exception,
                "Transfer permission is not valid, which may be caused by duplicated keys.");
        ...
        if(udact.manage.has_value()) {
            // manage permission's threshold can be 0 which means no one can update permission later.
            VAST_ASSERT(udact.manage->name == "manage", permission_type_exception,
                "Name ${name} does not match with the name of manage permission.", ("name",udact.manage->name));
            VAST_ASSERT(validate(*udact.manage), permission_type_exception,
                "Manage permission is not valid, which may be caused by duplicated keys.");
