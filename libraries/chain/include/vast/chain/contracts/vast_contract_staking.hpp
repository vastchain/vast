...
VAST_ACTION_IMPL_BEGIN(newstakepool) {
    using namespace internal;

    auto nsact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.fungible), name128::from_number(nsact.sym_id)), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        DECLARE_TOKEN_DB()

        VAST_ASSERT2(nsact.sym_id == VAST_SYM_ID, staking_symbol_exception, "Only VAST is supported to stake currently");
        VAST_ASSERT2(!tokendb.exists_token(token_type::stakepool, std::nullopt, nsact.sym_id), stakepool_duplicate_exception,
            "Stakepool with sym id: {} already exists.", nsact.sym_id);
        VAST_ASSERT2(nsact.sym_id == nsact.purchase_threshold.sym().id(), symbol_type_exception,
...
VAST_ACTION_IMPL_BEGIN(updstakepool) {
    using namespace internal;

    auto usact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.fungible), name128::from_number(usact.sym_id)), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        DECLARE_TOKEN_DB()

        VAST_ASSERT2(usact.sym_id == VAST_SYM_ID, staking_symbol_exception, "Only VAST is supported to stake currently");
        ...
        if(usact.purchase_threshold.has_value()) {
            VAST_ASSERT2(usact.sym_id == usact.purchase_threshold->sym().id(), symbol_type_exception,
                "Purchase threshold's symbol should match stake pool");
...
VAST_ACTION_IMPL_BEGIN(newvalidator) {
    using namespace internal;

    auto nvact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.staking), nvact.name), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        check_name_reserved(nvact.name);

        DECLARE_TOKEN_DB()
        VAST_ASSERT2(!tokendb.exists_token(token_type::validator, std::nullopt, nvact.name), validator_duplicate_exception,
            "validator {} already exists.", nvact.name);

        VAST_ASSERT(nvact.withdraw.name == "withdraw", permission_type_exception,
            "Name ${name} does not match with the name of withdraw permission.", ("name",nvact.withdraw.name));
        VAST_ASSERT(nvact.withdraw.threshold > 0 && validate(nvact.withdraw), permission_type_exception,
        ...
        VAST_ASSERT(nvact.manage.name == "manage", permission_type_exception,
            "Name ${name} does not match with the name of manage permission.", ("name",nvact.manage.name));
        VAST_ASSERT(validate(nvact.manage), permission_type_exception,
...
VAST_ACTION_IMPL_BEGIN(valiwithdraw) {
    using namespace internal;

    auto vwact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.staking), vwact.name), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        DECLARE_TOKEN_DB()

        auto sym = vwact.amount.sym();
        VAST_ASSERT2(sym == vast_sym(), staking_symbol_exception, "Only VAST is supported to withdraw currently");
        ...
        auto vaddr = get_validator_address(vwact.name, VAST_SYM_ID);

        try {
            transfer_fungible(context, vaddr, vwact.addr, vwact.amount, N(valiwithdraw), false /* pay charge */);
        }
        catch(balance_exception&) {
            VAST_THROW2(staking_exception, "Exceeds total bonus received.");
        }
    }
    VAST_CAPTURE_AND_RETHROW(tx_apply_exception);
}
...
VAST_ACTION_IMPL_BEGIN(staketkns) {
    using namespace internal;

    auto stact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.staking), stact.validator), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        DECLARE_TOKEN_DB()

        auto sym = stact.amount.sym();
        VAST_ASSERT2(sym == vast_sym(), staking_symbol_exception, "Only VAST is supported to stake currently");

        auto prop = property_stakes();
        READ_DB_ASSET(stact.staker, sym, prop);
        VAST_ASSERT2(prop.amount >= stact.amount.amount(), balance_exception, "Don't have enough balance to stake");

        switch(stact.type) {
        case stake_type::active: {
            VAST_ASSERT2(stact.fixed_days == 0, staking_days_exception, "Active staking cannot have fixed days");
            break;
        }
        case stake_type::fixed: {
            VAST_ASSERT2(stact.fixed_days > 0, staking_days_exception, "Fixed staking should have positive fixed days");
            break;
        }
        };  // switch
        ...
        VAST_ASSERT2(stact.amount.to_real() >= validator->current_net_value.to_real(), staking_amount_exception, "Needs to stake at least one unit");

        auto stakepool = make_empty_cache_ptr<stakepool_def>();
        READ_DB_TOKEN(token_type::stakepool, std::nullopt, sym.id(), stakepool, unknown_stakepool_exception,
            "Cannot find stakepool");

        VAST_ASSERT2(stact.amount >= stakepool->purchase_threshold, staking_amount_exception, "Needs to stake at least the same as purchase threshold in stakepool: {}", stakepool->purchase_threshold);
...
VAST_ACTION_IMPL_BEGIN(toactivetkns) {
    using namespace internal;
    namespace mp = boost::multiprecision;

    auto tatact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.staking), tatact.validator), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        DECLARE_TOKEN_DB()

        VAST_ASSERT2(tatact.sym_id == VAST_SYM_ID, staking_symbol_exception, "Only VAST is supported to stake currently");

        auto prop = property_stakes();
        READ_DB_ASSET(tatact.staker, vast_sym(), prop);
        ...
        for(auto& s : prop.stake_shares) {
            ...
            diff_amount   += (int64_t)mp::floor(s.net_value.to_real() * (new_uints - s.units) * std::pow(10, vast_sym().precision()));
        ...
        // update pool
        stakepool->total += asset(diff_amount, vast_sym());
...
VAST_ACTION_IMPL_BEGIN(unstaketkns) {
    using namespace internal;
    namespace mp = boost::multiprecision;

    auto ustact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.staking), ustact.validator), action_authorize_exception, "Invalid authorization fields in action(domain and key).");
        VAST_ASSERT2(ustact.sym_id == vast_sym().id(), staking_symbol_exception, "Only VAST is supported to unstake currently");

        DECLARE_TOKEN_DB()

        VAST_ASSERT2(ustact.units > 0, staking_units_exception, "Unstake units should be large than 0");

        auto prop = property_stakes();
        READ_DB_ASSET(ustact.staker, vast_sym(), prop);
        ...
        case unstake_op::propose: {
            ...
            VAST_ASSERT2(remainning_units == 0, staking_not_enough_exception, "Don't have enough staking units");
        ...
        case unstake_op::cancel: {
            ...
            VAST_ASSERT2(remainning_units == 0, staking_not_enough_exception, "Don't have enough pending staking units");
        ...
        case unstake_op::settle: {
            ...
            for(auto &s : prop.pending_shares) {
                ...
                // update amounts
                auto amount = (int64_t)mp::floor(s.net_value.to_real() * units * std::pow(10, vast_sym().precision()));
                auto diff   = (int64_t)mp::floor((validator->current_net_value.to_real() - s.net_value.to_real()) * units * std::pow(10, vast_sym().precision()));
            ...
            auto addr  = get_fungible_address(vast_sym());
            auto vaddr = get_validator_address(ustact.validator, VAST_SYM_ID);

            try {
                transfer_fungible(context, addr, ustact.staker, asset(bonus_amount, vast_sym()), N(unstaketkns), false /* pay charge */);
                transfer_fungible(context, addr, vaddr, asset(vbonus_amount, vast_sym()), N(unstaketkns), false /* pay charge */);
            }
            catch(balance_exception&) {
                VAST_THROW2(fungible_supply_exception, "Exceeds total supply of fungible with sym id: {}.", ustact.sym_id);
            }
            ...
            READ_DB_TOKEN(token_type::stakepool, std::nullopt, ustact.sym_id, stakepool, staking_exception,
                "Cannot find stakepool");
            stakepool->total -= asset(frozen_amount + bonus_amount, vast_sym());
...
VAST_ACTION_IMPL_BEGIN(recvstkbonus) {
    using namespace internal;
    namespace mp = boost::multiprecision;

    auto rsbact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.staking), rsbact.validator), action_authorize_exception, "Invalid authorization fields in action(domain and key).");
        VAST_ASSERT2(rsbact.sym_id == vast_sym().id(), staking_symbol_exception, "Only VAST is supported to unstake currently");
        ...
        VAST_ASSERT2(curr_block_num >= begin && curr_block_num < end, staking_timeing_exception,
        ...
        VAST_ASSERT2(validator->total_units >= conf.staking_threshold, staking_not_enough_exception, "Need at least {} units to start receiving staking rewards", conf.staking_threshold);
        ...
        auto diff_amount          = (int64_t)mp::round((new_net_value.to_real() - validator->current_net_value.to_real()) * validator->total_units * std::pow(10, vast_sym().precision()));
