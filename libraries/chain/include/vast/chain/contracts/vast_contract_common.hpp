...
#include <vast/chain/workaround/boost/safe_numerics/exception.hpp>
...
#include <vast/chain/apply_context.hpp>
...
#include <vast/chain/dense_hash.hpp>
...
#define VAST_ACTION_IMPL_BEGIN(name)                   \
    template<>                                        \
    struct apply_action<N(name)> {                    \
        template<typename ACT>                        \
        static void invoke(apply_context& context)

#define VAST_ACTION_IMPL_END() };

#define VAST_ACTION_VER() ACT::get_version()
...
validate(const group& group, const group::node& node) {
    VAST_ASSERT(node.validate(), group_type_exception, "Node is invalid: ${node}", ("node",node));
    if(!node.is_leaf()) {
...
validate(const group& group) {
    VAST_ASSERT(!group.name().empty(), group_type_exception, "Group name cannot be empty.");
    VAST_ASSERT(!group.empty(), group_type_exception, "Root node does not exist.");
    ...
    auto checker = [&](const auto& p, auto allowed_owner) {
        for(const auto& a : p.authorizers) {
            ...
            case authorizer_ref::owner_t: {
                VAST_ASSERT(allowed_owner, permission_type_exception,
            ...
            case authorizer_ref::group_t: {
                ...
                VAST_ASSERT(dbexisted, unknown_group_exception, "Group ${name} does not exist.", ("name", name));
                break;
            }
            case authorizer_ref::script_t: {
                ...
                VAST_ASSERT(dbexisted, unknown_script_exception, "Script ${name} does not exist.", ("name", name));
                break;
            }
            default: {
                VAST_ASSERT(false, authorizer_ref_type_exception, "Authorizer ref is not valid.");
            }
            }  // switch
        }
    };
...
check_name_reserved(const name128& name) {
    VAST_ASSERT(!name.empty() && !name.reserved(), name_reserved_exception, "Name starting with '.' is reserved for system usages.");
}
...
check_address_reserved(const address& addr, bool throw_on_reserved = true) {
    ...
    case address::reserved_t: {
        if(throw_on_reserved) {
            VAST_THROW(address_reserved_exception, "Address is reserved and cannot be used here");
        }
        return;
    }
...
template<>
name128
get_db_key<vast_link_object>(const vast_link_object& v) {
    return v.link_id;
}
...
#define PUT_DB_ASSET(ADDR, VALUE)                                     \
    {                                                                 \
        if constexpr(std::is_same_v<decltype(VALUE), property>) {     \
            assert(VALUE.sym.id() != VAST_SYM_ID);                     \
        }                                                             \
...
#define CHECK_SYM(VALUEREF, PROVIDED) \
    VAST_ASSERT2(VALUEREF.sym == PROVIDED, asset_symbol_exception, "Provided symbol({}) is invalid, expected: {}", PROVIDED, VALUEREF.sym);
...
#define READ_DB_ASSET(ADDR, SYM, VALUEREF)                                                              \
    try {                                                                                               \
        ...
        VAST_THROW2(balance_exception, "There's no balance left in {} with sym id: {}", ADDR, SYM.id()); \
    }                                                                                                   \
...
transfer_fungible_internal(apply_context& context,
    ...
    if constexpr(std::is_same_v<PROPERTY, property_stakes>) {
        READ_DB_ASSET(from, vast_sym(), pfrom);
    }
    else {
    ...
    // fast path check
    VAST_ASSERT2(pfrom.amount >= total.amount(), balance_exception,
    ...
    // vast and pvast cannot have passive bonus
    if(sym.id() > PVAST_SYM_ID && pay_bonus) {
    ...
    VAST_ASSERT2(pfrom.amount >= actual_amount, balance_exception,
    ...
    VAST_ASSERT(!r1.exception() && !r2.exception(), math_overflow_exception, "Opeartions resulted in overflows.");
    ...
    if(bonus_amount > 0) {
        ...
        VAST_ASSERT2(!r.exception(), math_overflow_exception, "Opeartions resulted in overflows.");
...
freeze_fungible(apply_context& context, const address& addr, asset total) {
    ...
    VAST_ASSERT2(prop.amount >= total.amount(), balance_exception,
...
unfreeze_fungible(apply_context& context, const address& addr, asset total) {
    ...
    VAST_ASSERT2(prop.frozen_amount >= total.amount(), balance_exception,
