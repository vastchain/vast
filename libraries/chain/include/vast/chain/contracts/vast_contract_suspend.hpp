...
VAST_ACTION_IMPL_BEGIN(newsuspend) {
    using namespace internal;

    auto nsact = context.act.data_as<newsuspend>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.suspend), nsact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");

        auto now = context.control.pending_block_time();
        VAST_ASSERT(nsact.trx.expiration > now, suspend_expired_tx_exception,
        ...
        for(auto& act : nsact.trx.actions) {
            VAST_ASSERT(act.domain != N128(suspend), suspend_invalid_action_exception, "Actions in 'suspend' domain are not allowd deferred-signning");
            VAST_ASSERT(act.name != N(everipay), suspend_invalid_action_exception, "everiPay action is not allowd deferred-signning");
            VAST_ASSERT(act.name != N(everipass), suspend_invalid_action_exception, "everiPass action is not allowd deferred-signning");
        }

        DECLARE_TOKEN_DB()
        VAST_ASSERT(!tokendb.exists_token(token_type::suspend, std::nullopt, nsact.name), suspend_duplicate_exception,
...
VAST_ACTION_IMPL_BEGIN(aprvsuspend) {
    using namespace internal;

    auto& aeact = context.act.data_as<add_clr_t<ACT>>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.suspend), aeact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
        ...
        VAST_ASSERT(suspend->status == suspend_status::proposed, suspend_status_exception,
        ...
        VAST_ASSERT(signed_keys == required_keys, suspend_not_required_keys_exception,
            "Provided keys are not required in this suspend transaction");
       
        for(auto it = signed_keys.cbegin(); it != signed_keys.cend(); it++) {
            VAST_ASSERT(suspend->signed_keys.find(*it) == suspend->signed_keys.end(), suspend_duplicate_key_exception,
...
VAST_ACTION_IMPL_BEGIN(cancelsuspend) {
    using namespace internal;

    auto& csact = context.act.data_as<add_clr_t<ACT>>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.suspend), csact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
...
VAST_ACTION_IMPL_BEGIN(execsuspend) {
    using namespace internal;

    auto& esact = context.act.data_as<add_clr_t<ACT>>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.suspend), esact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
        ...
        VAST_ASSERT(suspend->signed_keys.find(esact.executor) != suspend->signed_keys.end(), suspend_executor_exception,
            "Executor hasn't sign his key on this suspend transaction");

        auto now = context.control.pending_block_time();
        VAST_ASSERT(suspend->status == suspend_status::proposed, suspend_status_exception,
            "Suspend transaction is not in 'proposed' status.");
        VAST_ASSERT(suspend->trx.expiration > now, suspend_expired_tx_exception,
        ...
        if(suspend->trx.payer.type() == address::public_key_t) {
            VAST_ASSERT(suspend->signed_keys.find(suspend->trx.payer.get_public_key()) != suspend->signed_keys.end(), payer_exception,
