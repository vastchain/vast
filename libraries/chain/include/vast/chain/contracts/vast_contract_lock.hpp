...
VAST_ACTION_IMPL_BEGIN(newlock) {
    using namespace internal;

    auto nlact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.lock), nlact.name), action_authorize_exception, "Invalid authorization fields in action(domain and key).");

        DECLARE_TOKEN_DB()
        VAST_ASSERT(!tokendb.exists_token(token_type::lock, std::nullopt, nlact.name), lock_duplicate_exception,
            "Lock assets with same name: ${n} is already existed", ("n",nlact.name));

        auto now = context.control.pending_block_time();
        VAST_ASSERT(nlact.unlock_time > now, lock_unlock_time_exception,
            "Now is ahead of unlock time, unlock time is ${u}, now is ${n}", ("u",nlact.unlock_time)("n",now));
        VAST_ASSERT(nlact.deadline > now && nlact.deadline > nlact.unlock_time, lock_unlock_time_exception,
        ...
        case lock_type::cond_keys: {
            ...
            VAST_ASSERT(lck.threshold > 0 && lck.cond_keys.size() >= lck.threshold, lock_condition_exception,
        ...
        VAST_ASSERT(nlact.assets.size() > 0, lock_assets_exception, "Assets for lock should not be empty");
        ...
        for(auto& la : nlact.assets) {
            ...
            case asset_type::tokens: {
                ...
                VAST_ASSERT(tokens.names.size() > 0, lock_assets_exception, "NFT assets should be provided.");
            ...
            case asset_type::fungible: {
                ...
                VAST_ASSERT(fungible.amount.sym().id() != PVAST_SYM_ID, lock_assets_exception, "Pinned VAST cannot be used to be locked.");
                has_fungible = true;
        ...
        if(has_fungible) {
            ...
            VAST_ASSERT(nlact.succeed.size() == 1, lock_address_exception,
                "Size of address for succeed situation should be only one when there's fungible assets needs to lock");
            VAST_ASSERT(nlact.failed.size() == 1, lock_address_exception,
                "Size of address for failed situation should be only one when there's fungible assets needs to lock");
        }
        else {
            VAST_ASSERT(nlact.succeed.size() > 0, lock_address_exception, "Size of address for succeed situation should not be empty");
            VAST_ASSERT(nlact.failed.size() > 0, lock_address_exception, "Size of address for failed situation should not be empty");
        }
...
VAST_ACTION_IMPL_BEGIN(aprvlock) {
    using namespace internal;

    auto& alact = context.act.data_as<add_clr_t<ACT>>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.lock), alact.name), action_authorize_exception, "Invalid authorization fields in action(domain and key).");
        ...
        auto now = context.control.pending_block_time();
        VAST_ASSERT(lock->unlock_time > now, lock_expired_exception,
        ...
        case lock_type::cond_keys: {
            VAST_ASSERT(alact.data.type() == lock_aprv_type::cond_key, lock_aprv_data_exception,
                "Type of approve data is not conditional key");
            auto& lck = lock->condition.template get<lock_condkeys>();

            VAST_ASSERT(std::find(lck.cond_keys.cbegin(), lck.cond_keys.cend(), alact.approver) != lck.cond_keys.cend(),
                lock_aprv_data_exception, "Approver is not valid");
            VAST_ASSERT(lock->signed_keys.find(alact.approver) == lock->signed_keys.cend(), lock_duplicate_key_exception,
...
VAST_ACTION_IMPL_BEGIN(tryunlock) {
    using namespace internal;

    auto& tuact = context.act.data_as<add_clr_t<ACT>>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.lock), tuact.name), action_authorize_exception,
        ...
        auto now = context.control.pending_block_time();
        VAST_ASSERT(lock->unlock_time < now, lock_not_reach_unlock_time,
        ...
        if(pkeys == nullptr) {
            // not succeed
            VAST_ASSERT(lock->deadline < now, lock_not_reach_deadline,
