...
auto check_meta_key_reserved = [](const auto& key) {
    VAST_ASSERT(!key.reserved(), meta_key_exception, "Meta-key is reserved and cannot be used");
};

}  // namespace internal

VAST_ACTION_IMPL_BEGIN(addmeta) {
    ...
    try {
        ...
        if(act.domain == N128(.group)) {  // group
            ...
            VAST_ASSERT2(!check_duplicate_meta(*gp, amact.key), meta_key_exception,"Metadata with key: {} already exists.", amact.key);
            if(amact.creator.is_group_ref()) {
                VAST_ASSERT(amact.creator.get_group() == gp->name_, meta_involve_exception, "Only group itself can add its own metadata");
            }
            else {
                // check involved, only group manager(aka. group key) can add meta
                VAST_ASSERT(check_involved_group(*gp, amact.creator.get_account()), meta_involve_exception,
            ...
            if(amact.key.reserved()) {
                VAST_ASSERT(check_reserved_meta(amact, fungible_metas), meta_key_exception, "Meta-key is reserved and cannot be used");
            }
            ...
            VAST_ASSERT(!check_duplicate_meta(*fungible, amact.key), meta_key_exception,
            ...
            if(amact.creator.is_account_ref()) {
                ...
                VAST_ASSERT(involved, meta_involve_exception, "Creator is not involved in fungible: ${name}.", ("name",act.key));
            }
            else {
                // check involved, only group in `manage` permission can add meta
                VAST_ASSERT(check_involved_fungible(tokendb_cache, *fungible, N(manage), amact.creator), meta_involve_exception,
            ...
            if(amact.key.reserved()) {
                VAST_ASSERT(check_reserved_meta(amact, domain_metas), meta_key_exception, "Meta-key is reserved and cannot be used");
            }
            ...
            VAST_ASSERT(!check_duplicate_meta(*domain, amact.key), meta_key_exception,
                "Metadata with key ${key} already exists.", ("key",amact.key));
            // check involved, only person involved in `manage` permission can add meta
            VAST_ASSERT(check_involved_domain(tokendb_cache, *domain, N(manage), amact.creator), meta_involve_exception,
            ...
            VAST_ASSERT(!check_token_destroy(*token), token_destroyed_exception, "Metadata cannot be added on destroyed token.");
            VAST_ASSERT(!check_token_locked(*token), token_locked_exception, "Metadata cannot be added on locked token.");
            VAST_ASSERT(!check_duplicate_meta(*token, amact.key), meta_key_exception, "Metadata with key ${key} already exists.", ("key",amact.key));
            ...
            if(amact.creator.is_account_ref()) {
                ...
                VAST_ASSERT(involved, meta_involve_exception, "Creator is not involved in token ${domain}-${name}.", ("domain",act.domain)("name",act.key));
            }
            else {
...
VAST_ACTION_IMPL_BEGIN(paycharge) {
    using namespace internal;
    ...
    try {
        ...
        property_stakes vast, pvast;
        READ_DB_ASSET_NO_THROW_NO_NEW(pcact.payer, pvast_sym(), pvast);
        auto paid = std::min((int64_t)pcact.charge, pvast.amount);
        if(paid > 0) {
            pvast.amount -= paid;
            PUT_DB_ASSET(pcact.payer, pvast);
        }

        if(paid < pcact.charge) {
            READ_DB_ASSET_NO_THROW_NO_NEW(pcact.payer, vast_sym(), vast);
            auto remain = pcact.charge - paid;
            if(vast.amount < (int64_t)remain) {
                VAST_THROW2(charge_exceeded_exception,"There are only {} and {} left, but charge is {}",
                    asset(vast.amount, vast_sym()), asset(pvast.amount, pvast_sym()), asset(pcact.charge, vast_sym()));
            }
            vast.amount -= remain;
            PUT_DB_ASSET(pcact.payer, vast);
        }
        ...
        property_stakes bp;
        READ_DB_ASSET_NO_THROW(address(prod), vast_sym(), bp);
...
VAST_ACTION_IMPL_BEGIN(paybonus) {
    // empty body
...
auto update_chain_config = [](auto& conf, auto key, auto v) {
    ...
    default: {
        VAST_THROW2(prodvote_key_exception, "Configuration key: {} is not valid", key);
    }
    } // switch
};
...
VAST_ACTION_IMPL_BEGIN(prodvote) {
    using namespace internal;

    auto& pvact = context.act.data_as<add_clr_t<ACT>>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.prodvote), pvact.key), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
        VAST_ASSERT(pvact.value > 0 && pvact.value < 1'000'000, prodvote_value_exception, "Invalid prodvote value: ${v}", ("v",pvact.value));
        ...
        // test if it's action-upgrade vote and wheather action is valid
        {
            ...
            if(boost::starts_with(key, "action-")) {
                try {
                    ...
                    VAST_THROW2(prodvote_key_exception, "Invalid action name provided: {}", key.substr(7));
                }

                auto cver = exec_ctx.get_current_version(act);
                auto mver = exec_ctx.get_max_version(act);
                VAST_ASSERT2(pvact.value >= cver && pvact.value <= exec_ctx.get_max_version(act), prodvote_value_exception,
        ...
        VAST_ASSERT(pkey.has_value(), prodvote_producer_exception, "${p} is not a valid producer", ("p",pvact.producer));
        ...
        if(map == nullptr) {
            ...
            if(it.second == false) {
                // existed
                VAST_ASSERT2(it.first->second != pvact.value, prodvote_value_exception, "Value voted for {} is the same as previous voted", pvact.key);
...
VAST_ACTION_IMPL_BEGIN(updsched) {
    auto usact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.prodsched), N128(.update)), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
...
VAST_ACTION_IMPL_BEGIN(newscript) {
    using namespace internal;

    auto asact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.script), asact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");

        DECLARE_TOKEN_DB()
        VAST_ASSERT2(!tokendb.exists_token(token_type::script, std::nullopt, asact.name), script_duplicate_exception,
...
VAST_ACTION_IMPL_BEGIN(updscript) {
    using namespace internal;

    auto usact = context.act.data_as<ACT>();
    try {
        VAST_ASSERT(context.has_authorized(N128(.script), usact.name), action_authorize_exception,
            "Invalid authorization fields in action(domain and key).");
