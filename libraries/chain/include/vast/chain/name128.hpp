...
#define N128(X) vast::chain::string_to_name128(#X)
...
template <>
struct hash<vast::chain::name128> : private hash<uint128_t> {
    using argument_type = vast::chain::name128;
...
void to_variant(const vast::chain::name128& name, fc::variant& v);
void from_variant(const fc::variant& v, vast::chain::name128& name);
...
template <>
struct formatter<vast::chain::name128> {
    template <typename ParseContext>
    constexpr auto parse(ParseContext& ctx) { return ctx.begin(); }

    template <typename FormatContext>
    auto format(const vast::chain::name128& n, FormatContext &ctx) {
