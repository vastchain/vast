...
private:
    ...
    db_value(const T& v) : var_(std::in_place_index_t<kArray>()) {
        ...
        if(sz <= kInStakeSize) {
            if(auto arr = std::get_if<std::array<char, kInStakeSize>>(&var_)) {
    ...
    void
    set_view() {
        if(auto pa = std::get_if<std::array<char, kInStakeSize>>(&var_)) {
            view_ = std::string_view(pa->data(), pa->size());
        }
        else if(auto ps = std::get_if<std::string>(&var_)) {
            view_ = std::string_view(ps->data(), ps->size());
        }
        else {
            assert(false);
        }
...
public:
    struct config {
        ...
        fc::path        db_path           = ::vast::chain::config::default_token_database_dir_name;
...
FC_REFLECT(vast::chain::token_database::config, (profile)(block_cache_size)(object_cache_size)(db_path));
