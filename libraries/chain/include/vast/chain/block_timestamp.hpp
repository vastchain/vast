...
public:
    ...
    block_timestamp(const fc::time_point_sec& t) { set_time_point(t); }
    ...
    block_timestamp
    next() const {
        VAST_ASSERT(std::numeric_limits<uint32_t>::max() - slot >= 1, fc::overflow_exception, "block timestamp overflow");
    ...
    void
    operator=(const fc::time_point& t) {
        set_time_point(t);
    }
...
private:
    void
    set_time_point(const fc::time_point& t) {
    ...
    void
    set_time_point(const fc::time_point_sec& t) {
...
FC_REFLECT(vast::chain::block_timestamp_type, (slot))

namespace fc {
template <uint16_t IntervalMs, uint64_t EpochMs>
void
to_variant(const vast::chain::block_timestamp<IntervalMs, EpochMs>& t, fc::variant& v) {
    to_variant((fc::time_point)t, v);
}

template <uint16_t IntervalMs, uint64_t EpochMs>
void
from_variant(const fc::variant& v, vast::chain::block_timestamp<IntervalMs, EpochMs>& t) {
