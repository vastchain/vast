...
#else
#error VAST can only be compiled in X86-64 architecture
#endif
...
token_database_impl::open(int load_persistence) {
    ...
    VAST_ASSERT(db_ == nullptr, token_database_exception, "Token database is already opened");
    ...
    if(config_.profile == storage_profile::disk) {
        ...
        VAST_THROW(token_database_exception, "Unknown token database profile");
    }
    ...
    if(!fc::exists(config_.db_path)) {
        ...
        if(!status.ok()) {
            VAST_THROW(token_database_rocksdb_exception, "Rocksdb internal error: ${err}", ("err", status.getState()));
        }
...
token_database_impl::read_token(const name128& prefix, const name128& key, std::string& out, bool no_throw) const {
    ...
    if(!status.ok()) {
        ...
        if(!no_throw) {
            VAST_THROW(unknown_token_database_key, "Cannot find key: ${k} with prefix: ${p}", ("k",key)("p",prefix));
        }
        return false;
    }
    return true;
...
token_database_impl::read_asset(const address& addr, const symbol_id_type sym_id, std::string& out, bool no_throw) const {
    ...
    if(!status.ok()) {
        ...
        if(!no_throw) {
            VAST_THROW2(unknown_token_database_key, "There's no balance of fungible with sym id: {} in address: {}", sym_id, addr);
        }
        return false;
    }
    return true;
...
token_database_impl::add_savepoint(int64_t seq) {
    ...
    if(!savepoints_.empty()) {
        ...
        if(b.seq >= seq) {
            VAST_THROW(token_database_seq_not_valid, "Seq is not valid, prev: ${prev}, curr: ${curr}",
                      ("prev", b.seq)("curr", seq));
        }
    }
...
token_database_impl::pop_back_savepoint() {
    VAST_ASSERT(!savepoints_.empty(), token_database_no_savepoint, "There's no savepoints anymore");
...
token_database_impl::squash() {
    using namespace internal;
    VAST_ASSERT(savepoints_.size() >= 2, token_database_squash_exception, "Squash needs at least two savepoints.");
    
    auto n = savepoints_.back().node;
    VAST_ASSERT(n.f.type == kRuntime, token_database_squash_exception, "Squash needs two realtime savepoints.");

    savepoints_.pop_back();
    auto n2 = savepoints_.back().node;
    VAST_ASSERT(n2.f.type == kRuntime, token_database_squash_exception, "Squash needs two realtime savepoints.");
...
token_database_impl::persist_savepoints() const {
    ...
    VAST_CAPTURE_AND_RETHROW(token_database_persist_exception);
}
...
token_database_impl::load_savepoints(std::istream& is) {
    ...
    auto h = pd_header();
    fc::raw::unpack(is, h);
    VAST_ASSERT(h.dirty_flag == 0, token_database_dirty_flag_exception, "checkpoints log file dirty flag set");
...
FC_REFLECT(vast::chain::internal::pd_header, (dirty_flag));
FC_REFLECT(vast::chain::internal::pd_action, (op)(type)(key)(value));
FC_REFLECT(vast::chain::internal::pd_group,  (seq)(actions));
FC_REFLECT(vast::chain::internal::wc_entry, (k)(v));
FC_REFLECT(vast::chain::internal::wc_entry_pack, (seq)(vec));
