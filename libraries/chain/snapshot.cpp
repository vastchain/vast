...
variant_snapshot_reader::validate() const {
    VAST_ASSERT(snapshot.is_object(), snapshot_validation_exception,
               "Variant snapshot is not an object");
    const fc::variant_object& o = snapshot.get_object();

    VAST_ASSERT(o.contains("version"), snapshot_validation_exception,
               "Variant snapshot has no version");

    const auto& version = o["version"];
    VAST_ASSERT(version.is_integer(), snapshot_validation_exception,
               "Variant snapshot version is not an integer");

    VAST_ASSERT(version.as_uint64() == (uint64_t)current_snapshot_version, snapshot_validation_exception,
    ...
    VAST_ASSERT(o.contains("sections"), snapshot_validation_exception,
               "Variant snapshot has no sections");

    const auto& sections = o["sections"];
    VAST_ASSERT(sections.is_array(), snapshot_validation_exception, "Variant snapshot sections is not an array");

    const auto& section_array = sections.get_array();
    for(const auto& section : section_array) {
        VAST_ASSERT(section.is_object(), snapshot_validation_exception, "Variant snapshot section is not an object");

        const auto& so = section.get_object();
        VAST_ASSERT(so.contains("name"), snapshot_validation_exception,
                   "Variant snapshot section has no name");

        VAST_ASSERT(so["name"].is_string(), snapshot_validation_exception,
                   "Variant snapshot section name is not a string");

        VAST_ASSERT(so.contains("rows"), snapshot_validation_exception,
                   "Variant snapshot section has no rows");

        VAST_ASSERT(so["rows"].is_array(), snapshot_validation_exception,
...
variant_snapshot_reader::set_section(const string& section_name) {
    ...
    VAST_THROW(snapshot_exception, "Variant snapshot has no section named ${n}", ("n", section_name));
}

size_t
variant_snapshot_reader::get_section_size(const string& section_name) {
    VAST_THROW(snapshot_exception, "Variant snapshot doesn't support this feature");
}
...
ostream_snapshot_writer::write_start_section(const std::string& section_name) {
    ...
    VAST_ASSERT(section_pos == std::streampos(-1), snapshot_exception, "Attempting to write a new section without closing the previous section");
...
istream_snapshot_reader::validate() const {
    ...
    try {
        ...
        VAST_ASSERT(actual_totem == expected_totem, snapshot_exception,
        ...
        VAST_ASSERT(actual_version == expected_version, snapshot_exception,
...
istream_snapshot_reader::set_section(const string& section_name) {
    ...
    VAST_THROW(snapshot_exception, "Binary snapshot has no section named ${n}", ("n", section_name));
}
...
istream_snapshot_reader::build_section_indexes() {
    ...
    while(true) {
        ...
        while((c = snapshot.get()) != '\0') {
            VAST_ASSERT(c != std::char_traits<char>::eof(), snapshot_validation_exception, "Not valid section name");
            name.push_back(c);
        }
