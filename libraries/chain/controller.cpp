...
#include <vast/chain/authority_checker.hpp>
#include <vast/chain/block_log.hpp>
#include <vast/chain/charge_manager.hpp>
#include <vast/chain/chain_snapshot.hpp>
#include <vast/chain/execution_context_impl.hpp>
#include <vast/chain/fork_database.hpp>
#include <vast/chain/snapshot.hpp>
#include <vast/chain/token_database.hpp>
#include <vast/chain/token_database_cache.hpp>
#include <vast/chain/token_database_snapshot.hpp>
#include <vast/chain/transaction_context.hpp>
...
#include <vast/chain/block_summary_object.hpp>
#include <vast/chain/global_property_object.hpp>
#include <vast/chain/transaction_object.hpp>
#include <vast/chain/reversible_block_object.hpp>
...
struct controller_impl {
    ...
    vast_execution_context    exec_ctx;
    ...
    pop_block() {
        ...
        VAST_ASSERT(prev, block_validate_exception, "attempt to pop beyond last irreversible block");
        ...
        if(read_mode == db_read_mode::SPECULATIVE) {
            VAST_ASSERT(head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
    ...
    controller_impl(const controller::config& cfg, controller& s)
        ...
        , system_api(contracts::vast_contract_abi(), cfg.max_serialization_time) {
    ...
    on_irreversible(const block_state_ptr& s) {
        ...
        if(!log_head) {
            if(s->block) {
                VAST_ASSERT(s->block_num == blog.first_block_num(), block_log_exception, "block log has no blocks and is appending the wrong first block.  Expected ${expected}, but received: ${actual}",
                           ("expected", fmt::format("{:n}", blog.first_block_num()))("actual", fmt::format("{:n}", s->block_num)));
                append_to_blog = true;
            }
            else {
                VAST_ASSERT(s->block_num == blog.first_block_num() - 1, block_log_exception, "block log has no blocks and is not properly set up to start after the snapshot");
            }
        }
        else {
            auto lh_block_num = log_head->block_num();
            if(s->block_num > lh_block_num) {
                VAST_ASSERT(s->block_num - 1 == lh_block_num, unlinkable_block_exception, "unlinkable block", ("s->block_num",fmt::format("{:n}", s->block_num))("lh_block_num",fmt::format("{:n}", lh_block_num)));
                VAST_ASSERT(s->block->previous == log_head->id(), unlinkable_block_exception, "irreversible doesn't link to block log head");
                append_to_blog = true;
            }
        }
        ...
        if(s->block) {
            if(read_mode == db_read_mode::IRREVERSIBLE) {
                ...
                if(!head || s->block_num == head->block_num + 1) {
                    ...
                    VAST_ASSERT(!head || head->block_num == 1, block_validate_exception, "Attempting to re-apply an irreversible block that was not the implied genesis block");
                }
    ...
    init(const snapshot_reader_ptr& snapshot) {
        ...
        if(snapshot) {
            VAST_ASSERT(!head, fork_database_exception, "");
            snapshot->validate();
            ...
            if(!end) {
                ...
                VAST_ASSERT(end->block_num() == head->block_num, fork_database_exception,
        ...
        if(objitr != ubi.rend()) {
            VAST_ASSERT(objitr->blocknum == head->block_num, fork_database_exception,
            ...
            auto end = blog.read_head();
            VAST_ASSERT(!end || end->block_num() == head->block_num, fork_database_exception,
                       "fork database exists but reversible block database does not, replay blockchain",
                       ("blog_head", end->block_num())("head", head->block_num));
        }

        VAST_ASSERT(db.revision() >= head->block_num, fork_database_exception, "fork database is inconsistent with shared memory",
        ...
        if(db.revision() > head->block_num) {
            wlog("warning: database revision (${db}) is greater than head block number (${head}), "
                 ...
                 ("db", db.revision())("head", head->block_num));
            VAST_ASSERT(token_db.savepoints_size() > 0, token_database_exception,
                "token database is inconsistent with fork database: don't have any savepoints to pop");
            VAST_ASSERT(token_db.latest_savepoint_seq() == db.revision(), token_database_exception,
        ...
        // add workaround to vast & pvast in vast-3.3.2
        update_vast_org(token_db, conf.genesis);
    }
    ...
    void
    initialize_token_db() {
        initialize_vast_org(token_db, conf.genesis);
    }
    ...
    commit_block(bool add_to_fork_db) {
        ...
        try {
            if(add_to_fork_db) {
                ...
                head = fork_db.head();
                VAST_ASSERT(new_bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
            }
    ...
    check_authorization(const public_keys_set& signed_keys, const transaction& trx) {
        ...
        for(const auto& act : trx.actions) {
            VAST_ASSERT(checker.satisfied(act), unsatisfied_authorization,
    ...
    push_transaction(const transaction_metadata_ptr& trx,
        ...
        VAST_ASSERT(deadline != fc::time_point(), transaction_exception, "deadline cannot be uninitialized");
    ...
    start_block(block_timestamp_type when, uint16_t confirm_block_count, controller::block_status s, const optional<block_id_type>& producer_block_id) {
        VAST_ASSERT(!pending.has_value(), block_validate_exception, "pending block already exists");
        ...
        if(!self.skip_db_sessions(s)) {
            VAST_ASSERT(db.revision() == head->block_num, database_exception, "db revision is not on par with head block",
    ...
    apply_block(const signed_block_ptr& b, controller::block_status s) {
        try {
            try {
                VAST_ASSERT(b->block_extensions.size() == 0, block_validate_exception, "no supported extensions");
                ...
                for(const auto& receipt : b->transactions) {
                    ...
                    if(receipt.type == transaction_receipt::input) {
                        ...
                        VAST_THROW(block_validate_exception, "encountered unexpected receipt type");
                    }
                    ...
                    VAST_ASSERT(pending->_pending_block_state->block->transactions.size() > 0,
                               block_validate_exception, "expected a receipt",
                               ("block", *b)("expected_receipt", receipt)
                               );
                    VAST_ASSERT(pending->_pending_block_state->block->transactions.size() == num_pending_receipts + 1,
                               block_validate_exception, "expected receipt was not added",
                               ("block", *b)("expected_receipt", receipt)
                               );
                    auto& r = pending->_pending_block_state->block->transactions.back();
                    VAST_ASSERT(r == static_cast<const transaction_receipt_header&>(receipt),
                ...
                VAST_ASSERT(producer_block_id == pending->_pending_block_state->header.id(),
    ...
    push_block(const signed_block_ptr& b) {
        ...
        VAST_ASSERT(!pending.has_value(), block_validate_exception, "it is not valid to push a block when there is a pending block");
        ...
        try {
            VAST_ASSERT(b, block_validate_exception, "trying to push empty block");
            VAST_ASSERT(s != controller::block_status::incomplete, block_validate_exception, "invalid block status for a completed block");
    ...
    maybe_switch_forks(controller::block_status s = controller::block_status::complete) {
        ...
        if(new_head->header.previous == head->id) {
            ...
            VAST_ASSERT(self.head_block_id() == branches.second.back()->header.previous, fork_database_exception,
    ...
    void
    finalize_block() {
        VAST_ASSERT(pending.has_value(), block_validate_exception, "it is not valid to finalize when there is no pending block");
        try {
            set_action_merkle();
            set_trx_merkle();
    ...
    void
    check_and_update_staking_ctx() {
        VAST_ASSERT(pending.has_value(), block_validate_exception, "it is not valid to check and update staking context when there is no pending block");
...
controller::push_transaction(const transaction_metadata_ptr& trx, fc::time_point deadline) {
    validate_db_available_size();
    VAST_ASSERT(get_read_mode() != chain::db_read_mode::READ_ONLY, transaction_type_exception, "push transaction not allowed in read-only mode");
    VAST_ASSERT(trx && !trx->implicit, transaction_type_exception, "Implicit transaction not allowed");
...
controller::pending_block_time() const {
    VAST_ASSERT(my->pending.has_value(), block_validate_exception, "no pending block");
...
controller::get_block_id_for_num(uint32_t block_num) const {
    try {
        ...
        VAST_ASSERT(BOOST_LIKELY(signed_blk != nullptr), unknown_block_exception,
...
controller::get_link_obj_for_link_id(const link_id_type& link_id) const {
    vast_link_object link_obj;

    auto str = std::string();
    try {
        my->token_db.read_token(token_type::vastlink, std::nullopt, link_id, str);
    }
    catch(token_database_exception&) {
        VAST_THROW2(vast_link_existed_exception, "Cannot find VastLink with id: {}", fc::to_hex((char*)&link_id, sizeof(link_id)));
    }
...
controller::get_block_num_for_trx_id(const transaction_id_type& trx_id) const {
    ...
    VAST_THROW(unknown_transaction_exception, "Transaction: ${t} is not existed", ("t",trx_id));
}
...
controller::write_snapshot(const snapshot_writer_ptr& snapshot) const {
    VAST_ASSERT(!my->pending.has_value(), block_validate_exception, "cannot take a consistent snapshot with a pending block");
...
controller::get_unapplied_transactions() const {
    if(my->read_mode != db_read_mode::SPECULATIVE) {
        VAST_ASSERT(my->unapplied_transactions.empty(), transaction_exception,
...
controller::validate_expiration(const transaction& trx) const {
    try {
        ...
        VAST_ASSERT(time_point(trx.expiration) >= pending_block_time(),
                   expired_tx_exception,
                   "transaction has expired, "
                   "expiration is ${trx.expiration} and pending block time is ${pending_block_time}",
                   ("trx.expiration", trx.expiration)("pending_block_time", pending_block_time()));
        VAST_ASSERT(time_point(trx.expiration) <= pending_block_time() + fc::seconds(chain_configuration.max_transaction_lifetime),
...
controller::validate_tapos(const transaction& trx) const {
    try {
        ...
        VAST_ASSERT(trx.verify_reference_block(tapos_block_summary.block_id), invalid_ref_block_exception,
...
controller::validate_db_available_size() const {
   ...
   VAST_ASSERT(free >= guard, database_guard_exception, "database free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
controller::validate_reversible_available_size() const {
   ...
   VAST_ASSERT(free >= guard, reversible_guard_exception, "reversible free: ${f}, guard size: ${g}", ("f", free)("g",guard));
}
...
controller::get_required_keys(const transaction& trx, const public_keys_set& candidate_keys) const {
    ...
    if(trx.payer.type() == address::public_key_t) {
        VAST_ASSERT(checker.satisfied_key(trx.payer.get_public_key()), unsatisfied_authorization, "Payer authorized failed");
    }
...
controller::get_suspend_required_keys(const proposal_name& name, const public_keys_set& candidate_keys) const {
    ...
    try {
        ...
        VAST_THROW2(unknown_suspend_exception, "Cannot find suspend proposal: {}", name);
    }
...
public_keys_set
controller::get_vastlink_signed_keys(const link_id_type& link_id) const {
    ...
    VAST_THROW2(vast_link_existed_exception, "Cannot find VastLink");
}
