...
symbol::from_string(const string& from) {
    try {
        ...
        auto c = s.find(',');
        VAST_ASSERT(c != string::npos, symbol_type_exception, "Symbol's precision and id should be separated with comma");
        ...
        VAST_ASSERT(id <= std::numeric_limits<uint32_t>::max(), symbol_type_exception, "Exceed max symbol id allowed");

        return symbol(p, id);
    }
    VAST_CAPTURE_AND_RETHROW(symbol_type_exception, (from));
}
...
asset::from_string(const string& from) {
    ...
    try {
        ...
        VAST_ASSERT((space_pos != string::npos), asset_type_exception,
        ...
        if(dot_pos != string::npos) {
            VAST_ASSERT((dot_pos != amount_str.size() - 1), asset_type_exception,
    ...
    VAST_CAPTURE_AND_RETHROW(asset_type_exception, (from));
}
