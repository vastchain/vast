...
fork_database::set(block_state_ptr s) {
    ...
    VAST_ASSERT(s->id == s->header.id(), fork_database_exception, "block state id (${id}) is different from block state header id (${hid})", ("id", string(s->id))("hid", string(s->header.id())));

    // VAST_ASSERT( s->block_num == s->header.block_num() );

    VAST_ASSERT(result.second, fork_database_exception, "unable to insert block state, duplicate state detected");
    if(!my->head) {
        my->head = s;
    }
...
fork_database::add(const block_state_ptr& n, bool skip_validate_previous) {
    VAST_ASSERT(n, fork_database_exception, "attempt to add null block state");
    VAST_ASSERT(my->head, fork_db_block_not_found, "no head block set");

    if(!skip_validate_previous) {
        auto prior = my->index.find(n->block->previous);
        VAST_ASSERT(prior != my->index.end(), unlinkable_block_exception,
            "unlinkable block", ("id", n->block->id())("previous", n->block->previous));
    }

    auto inserted = my->index.insert(n);
    VAST_ASSERT(inserted.second, fork_database_exception, "duplicate block added?");
...
fork_database::add(signed_block_ptr b, bool skip_validate_signee) {
    VAST_ASSERT(b, fork_database_exception, "attempt to add null block");
    VAST_ASSERT(my->head, fork_database_exception, "no head block set");

    const auto& by_id_idx = my->index.get<by_block_id>();
    auto        existing  = by_id_idx.find(b->id());
    VAST_ASSERT(existing == by_id_idx.end(), fork_database_exception, "we already know about this block");

    auto prior = by_id_idx.find(b->previous);
    VAST_ASSERT(prior != by_id_idx.end(), unlinkable_block_exception, "unlinkable block", ("id", b->id())("previous", b->previous));

    auto result = std::make_shared<block_state>(**prior, move(b), skip_validate_signee);
    VAST_ASSERT(result, fork_database_exception , "fail to add new block state");
...
fork_database::fetch_branch_from(const block_id_type& first, const block_id_type& second) const {
    ...
    while(first_branch->block_num > second_branch->block_num) {
        ...
        VAST_ASSERT(first_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(first_branch->header.previous)));
    }

    while(second_branch->block_num > first_branch->block_num) {
        result.second.push_back(second_branch);
        second_branch = get_block(second_branch->header.previous);
        VAST_ASSERT(second_branch, fork_db_block_not_found, "block ${id} does not exist", ("id", string(second_branch->header.previous)));
    }
    ...
    while(first_branch->header.previous != second_branch->header.previous) {
        ...
        first_branch  = get_block(first_branch->header.previous);
        second_branch = get_block(second_branch->header.previous);
        VAST_ASSERT(first_branch && second_branch, fork_db_block_not_found,
...
void
fork_database::set_validity(const block_state_ptr& h, bool valid) {
    if(!valid) {
        remove(h->id);
    }
    else {
...
void
fork_database::mark_in_current_chain(const block_state_ptr& h, bool in_current_chain) {
    if(h->in_current_chain == in_current_chain)
        return;

    auto& by_id_idx = my->index.get<by_block_id>();
    auto  itr       = by_id_idx.find(h->id);
    VAST_ASSERT(itr != by_id_idx.end(), fork_db_block_not_found, "could not find block in fork database");
...
void
fork_database::prune(const block_state_ptr& h) {
...
block_state_ptr
fork_database::get_block_in_current_chain_by_num(uint32_t n) const {
    ...
    // VAST_ASSERT( nitr != numidx.end() && (*nitr)->block_num == n,
    //           "could not find block in fork database with block number ${block_num}", ("block_num", n) );
    // VAST_ASSERT( (*nitr)->in_current_chain == true,
...
fork_database::add(const header_confirmation& c) {
    ...
    VAST_ASSERT(b, fork_db_block_not_found, "unable to find block id ${id}", ("id", c.block_id));
...
fork_database::set_bft_irreversible(block_id_type id) {
    ...
    idx.modify(itr, [&](auto& bsp) { bsp->bft_irreversible_blocknum = bsp->block_num; });
