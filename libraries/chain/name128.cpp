 ...
 */
#include <vast/chain/name128.hpp>
...
name128::set(const char* str) {
    ...
    VAST_ASSERT(len <= 21, name128_type_exception, "Name128 is longer than 21 characters (${name}) ",
               ("name", std::string(str)));
    VAST_ASSERT(len > 0, name128_type_exception, "Name128 cannot be empty");
    value = string_to_name128(str);
    VAST_ASSERT(to_string() == std::string(str), name128_type_exception,
...
name128::set(const std::string& str) {
    ...
    VAST_ASSERT(to_string() == str, name128_type_exception,
               "Name128 not properly normalized (name: ${name}, normalized: ${normalized}) ",
...
void
to_variant(const vast::chain::name128& name, fc::variant& v) {
    v = std::string(name);
}
void
from_variant(const fc::variant& v, vast::chain::name128& name) {
    name = v.get_string();
}
