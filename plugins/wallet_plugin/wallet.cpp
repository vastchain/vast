...
private_key_type
derive_private_key(const std::string& prefix_string,
                   int                sequence_number) {
...
public:
    ...
    bool
    copy_wallet_file(string destination_filename) {
    ...
    string
    get_wallet_filename() const { return _wallet_filename; }

    std::optional<private_key_type>
    ...
    get_private_key(const public_key_type& id) const {
        ...
        VAST_ASSERT(has_key, chain::key_nonexistent_exception, "Key doesn't exist!");
        return *has_key;
    }
    ...
    import_key(string wif_key) {
        ...
        vast::chain::public_key_type wif_pub_key = priv.get_public_key();
        ...
        VAST_THROW(chain::key_exist_exception, "Key already in wallet");
    }
    ...
    remove_key(string key)
    {
        ...
        VAST_THROW(chain::key_nonexistent_exception, "Key not in wallet");
    }

    string
    create_key(string key_type) {
        if(key_type.empty())
        ...
        if(key_type == "K1")
            ...
            VAST_THROW(chain::unsupported_key_type_exception, "Key type \"${kt}\" not supported by software wallet", ("kt", key_type));
    ...
    save_wallet_file(string wallet_filename = "") {
        ...
        try {
            ...
            if(!outfile) {
                ...
                VAST_THROW(wallet_exception, "Unable to open file: ${fn}", ("fn", wallet_filename));
            }
...
}}}  // namespace vast::wallet::detail

namespace vast { namespace wallet {
...
soft_wallet::import_key(string wif_key) {
    VAST_ASSERT(!is_locked(), wallet_locked_exception, "Unable to import key on a locked wallet");
...
soft_wallet::lock() {
    try {
        VAST_ASSERT(!is_locked(), wallet_locked_exception, "Unable to lock a locked wallet");
        encrypt_keys();
...
soft_wallet::unlock(string password) {
    ...
    VAST_RETHROW_EXCEPTIONS(chain::wallet_invalid_password_exception,
...
soft_wallet::set_password(string password) {
    if(!is_new())
        VAST_ASSERT(!is_locked(), wallet_locked_exception, "The wallet must be unlocked before the password can be set");
...
soft_wallet::list_keys() {
    VAST_ASSERT(!is_locked(), wallet_locked_exception, "Unable to list public keys of a locked wallet");
    return my->_keys;
}
...
soft_wallet::get_private_key_from_password(string account, string role, string password) const {
    ...
    VAST_ASSERT(seed.size(), wallet_exception, "seed should not be empty");
