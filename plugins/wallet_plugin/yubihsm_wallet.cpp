...
#include <vast/wallet_plugin/yubihsm_wallet.hpp>
#include <vast/chain/exceptions.hpp>
#include <vast/wallet_plugin/yubihsm.h>
...
struct yubihsm_wallet_impl {
    ...
    key_map_type::iterator
    populate_key_map_with_keyid(const uint16_t key_id) {
        yh_rc   rc;
        size_t  blob_sz = 128;
    ...
    create() {
        ...
        try {
            if((rc = api.util_generate_key_ec(session, &new_key_id, "vastwd created key", authkey_domains, &creation_caps, YH_ALGO_EC_P256)))
...
yubihsm_wallet::yubihsm_wallet(const string& connector, const uint16_t authkey)
    : my(new detail::yubihsm_wallet_impl(connector, authkey)) {
}
