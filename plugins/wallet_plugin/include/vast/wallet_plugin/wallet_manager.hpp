...
#include <vast/wallet_plugin/wallet_api.hpp>
...
public:
    ...
    void
    set_timeout(int64_t secs) { set_timeout(std::chrono::seconds(secs)); }
    ...
    /// @return A set of signatures from all unlocked wallets, all signed chain_id
    flat_set<signature_type> get_my_signatures(const chain_id_type& chain_id);
