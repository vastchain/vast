...
#include <vast/wallet_plugin/wallet_manager.hpp>
#include <vast/wallet_plugin/wallet.hpp>
#include <vast/wallet_plugin/se_wallet.hpp>
...
wallet_manager::set_timeout(const std::chrono::seconds& t) {
    ...
    VAST_ASSERT(timeout_time >= now && timeout_time.time_since_epoch().count() > 0, invalid_lock_timeout_exception, "Overflow on timeout_time, specified ${t}, now ${now}, timeout_time ${timeout_time}",
...
wallet_manager::create(const std::string& name) {
    ...
    VAST_ASSERT(valid_filename(name), wallet_exception, "Invalid filename, path not allowed in wallet name ${n}", ("n", name));

    auto wallet_filename = dir / (name + file_ext);

    if(fc::exists(wallet_filename)) {
        VAST_THROW(chain::wallet_exist_exception, "Wallet with name: '${n}' already exists at ${path}", ("n", name)("path", fc::path(wallet_filename)));
    }
    ...
    // This can happen if the wallet file is removed while vastd is running.
...
wallet_manager::open(const std::string& name) {
    ...
    if(!wallet->load_wallet_file()) {
        VAST_THROW(chain::wallet_nonexistent_exception, "Unable to open file: ${f}", ("f", wallet_filename.string()));
    }

    // If we have name in our map then remove it since we want the emplace below to replace.
    // This can happen if the wallet file is added while vastd is running.
    auto it = wallets.find(name);
    if(it != wallets.end()) {
        wallets.erase(it);
    }
...
wallet_manager::list_keys(const string& name, const string& pw) {
    ...
    if(wallets.count(name) == 0)
        VAST_THROW(chain::wallet_nonexistent_exception, "Wallet not found: ${w}", ("w", name));
    auto& w = wallets.at(name);
    if(w->is_locked())
        VAST_THROW(chain::wallet_locked_exception, "Wallet is locked: ${w}", ("w", name));
...
wallet_manager::get_public_keys() {
    check_timeout();
    VAST_ASSERT(!wallets.empty(), wallet_not_available_exception, "You don't have any wallet!");
    ...
    VAST_ASSERT(!is_all_wallet_locked, wallet_locked_exception, "You don't have any unlocked wallet!");
    return result;
}
...
wallet_manager::unlock(const std::string& name, const std::string& password) {
    ...
    auto& w = wallets.at(name);
    if(!w->is_locked()) {
        VAST_THROW(chain::wallet_unlocked_exception, "Wallet is already unlocked: ${w}", ("w", name));
        return;
    }
    w->unlock(password);
}
...
wallet_manager::sign_transaction(const chain::signed_transaction& txn, const flat_set<public_key_type>& keys, const chain::chain_id_type& id) {
    ...
    for(const auto& pk : keys) {
        ...
        if(!found) {
            VAST_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", pk));
        }
    }
...
wallet_manager::sign_digest(const chain::digest_type& digest, const public_key_type& key) {
    ...
    VAST_THROW(chain::wallet_missing_pub_key_exception, "Public key not found in unlocked wallets ${k}", ("k", key));
}

void
wallet_manager::own_and_use_wallet(const string& name, std::unique_ptr<wallet_api>&& wallet) {
    if(wallets.find(name) != wallets.end()) {
        VAST_THROW(wallet_exception, "Tried to use wallet name that already exists.");
    }
    wallets.emplace(name, std::move(wallet));
}
    ...
    t->async_wait([t, this](const boost::system::error_code& /*ec*/) {
        ...
        if(ec != boost::system::error_code()) {
            if(rc.type() == bfs::file_not_found) {
                appbase::app().quit();
                VAST_THROW(wallet_exception, "Lock file removed while vastwd still running. Terminating...");
            }
        }
...
wallet_manager::initialize_lock() {
    //This is technically somewhat racy in here -- if multiple vastd are in this function at once.
    ...
    lock_path = dir / "wallet.lock";
    {
        ...
        VAST_ASSERT(!x.fail(), wallet_exception, "Failed to open wallet lock file at ${f}", ("f", lock_path.string()));
    }
    ...
    if(!wallet_dir_lock->try_lock()) {
        ...
        VAST_THROW(wallet_exception, "Failed to lock access to wallet directory; is another vastwd running?");
    }
