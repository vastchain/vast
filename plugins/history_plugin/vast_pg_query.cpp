 ...
 */
#include <vast/history_plugin/vast_pg_query.hpp>
...
pg_query::send_once() {
    ...
    try {
        VAST_THROW2(chain::postgres_send_exception,
...
pg_query::poll_read() {
    ...
    while(1) {
        ...
        VAST_ASSERT(r, chain::postgres_poll_exception, "Poll messages from postgres failed, detail: ${d}", ("d",PQerrorMessage(conn_)));
...
pg_query::get_tokens_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get tokens failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg_query::get_domains_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get domains failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg_query::get_groups_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get groups failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg_query::get_fungibles_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get fungibles failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg_query::get_actions_async(int id, const read_only::get_actions_params& params) {
    ...
    if(params.take.has_value()) {
        t = *params.take;
        VAST_ASSERT(t <= 20, chain::exceed_query_limit_exception, "Exceed limit of max actions return allowed for each query, limit: 20 per query");
    }
...
pg_query::get_actions_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get actions failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
    auto n = PQntuples(r);
    if(n == 0) {
        return response_ok(id, std::string("[]")); // return empty
    }
...
auto gfa_plan0 = R"sql(SELECT transactions.trx_id, name, domain, key, data, transactions.timestamp
                       ...
                       WHERE
                           domain = '.fungible'
                           AND key = $1
                           AND name = ANY('{{"issuefungible","transferft","recycleft","vast2pvast","everipay","paybonus"}}')
...
pg_query::get_fungible_actions_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get fungible actions failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
#define READ_DB_ASSET(ADDR, SYM_ID, VALUEREF)                                                         \
    try {                                                                                             \
        ...
        VAST_THROW2(balance_exception, "There's no balance left in {} with sym id: {}", ADDR, SYM_ID); \
    }
...
pg_query::get_fungibles_balance_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get transaction failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg_query::get_transaction_resume(int id, pg_result const* r) {
    ...
    auto n = PQntuples(r);
    if(n == 0) {
        VAST_THROW(chain::unknown_transaction_exception, "Cannot find transaction");
    }
    ...
    VAST_THROW(chain::unknown_transaction_exception, "Cannot find transaction: ${t}", ("t", trx_id));
}
...
pg_query::get_fungible_ids_async(int id, const read_only::get_fungible_ids_params& params) {
    ...
    if(params.take.has_value()) {
        t = *params.take;
        VAST_ASSERT(t <= 100, chain::exceed_query_limit_exception, "Exceed limit of max actions return allowed for each query, limit: 100 per query");
    }
...
pg_query::get_fungible_ids_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get fungible ids failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg_query::get_transaction_actions_resume(int id, pg_result const* r) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_query_exception, "Get transaction actions failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
