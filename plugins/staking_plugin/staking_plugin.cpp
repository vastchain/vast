 ...
 */
#include <vast/staking_plugin/staking_plugin.hpp>
...
public:
    ...
    struct staking_config {
        ...
        fc::microseconds vastwd_provider_timeout_us;
...
staking_plugin::set_program_options(options_description&, options_description& cfg) {
    cfg.add_options()
        ...
        ("staking-vastwd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5), "Limits the maximum time (in milliseconds) that is allowed for pushing staking trx to a vastwd provider for signing")
    ;
}
...
static staking_plugin_impl::signature_provider_type
make_vastwd_signature_provider(const std::shared_ptr<staking_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
    ...
    return [weak_impl, vastwd_url, pubkey](const chain::digest_type& digest) {
        ...
        if(impl) {
            ...
            auto deadline = impl->config_.vastwd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->config_.vastwd_provider_timeout_us : fc::time_point::maximum();
...
staking_plugin::plugin_initialize(const variables_map& options) {
    ...
    try {
            ...
            for(const auto& key_spec_pair : key_spec_pairs) {
                try {
                    ...
                    if(spec_type_str == "KEY") {
                        ...
                        config.signature_providers[pubkey] = make_vastwd_signature_provider(my_, spec_data, pubkey);
                    }
                    else {
                        VAST_THROW(plugin_config_exception, "Invalid key provider");
                    }
                }
                catch(...) {
        ...
        VAST_ASSERT(config.signature_providers.find(config.payer) != config.signature_providers.cend(),
            plugin_config_exception, "Must provide signature provider for payer");

        config.vastwd_provider_timeout_us = fc::milliseconds(options.at("staking-vastwd-provider-timeout").as<int32_t>());
    }
    FC_LOG_AND_RETHROW();
