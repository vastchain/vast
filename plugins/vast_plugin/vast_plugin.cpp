...
static appbase::abstract_plugin& _vast_plugin = app().register_plugin<vast_plugin>();

using namespace vast;
using namespace vast::chain;

class vast_plugin_impl {
public:
    vast_plugin_impl(controller& db)
        : db_(db) {}
...
vast_plugin::vast_plugin() {}
vast_plugin::~vast_plugin() {}

void vast_plugin::set_program_options(options_description& cli, options_description& cfg) {}

void vast_plugin::plugin_initialize(const variables_map& options) {}

void
vast_plugin::plugin_startup() {
    this->my_.reset(new vast_plugin_impl(app().get_plugin<chain_plugin>().chain()));
}

void vast_plugin::plugin_shutdown() {}

vast_apis::read_only
vast_plugin::get_read_only_api() const {
    return vast_apis::read_only(my_->db_);
}

vast_apis::read_write
vast_plugin::get_read_write_api() {
    return vast_apis::read_write();
}
...
#define READ_DB_ASSET(ADDR, SYM, VALUEREF)                                                         \
    try {                                                                                          \
        ...
        VAST_THROW2(balance_exception, "There's no balance left in {} with sym id: {}", ADDR, SYM); \
    }
...
read_only::get_fungible_balance(const get_fungible_balance_params& params) {
    ...
    VAST_THROW(unsupported_feature, "Read all the balance of fungibles tokens within one address is not supported in vast_plugin anymore, please refer to the history_plugin");
}
...
read_only::get_validator(const get_validator_params& params) {
    ...
    property prop;
    READ_DB_ASSET_NO_THROW(address(N(.validator), validator->name, VAST_SYM_ID), symbol(5,VAST_SYM_ID), prop);
...
read_only::get_staking_shares(const get_staking_shares_params& params) {
    ...
    READ_DB_ASSET(params.address, vast_sym(), prop);

    fc::to_variant(prop, var);
    return var;
}

read_only::get_vastlink_signed_keys_result
read_only::get_vastlink_signed_keys(const get_vastlink_signed_keys_params& params) const {
    ...
    auto result        = get_vastlink_signed_keys_result();
    result.signed_keys = db_.get_vastlink_signed_keys(link_id);
    return result;
}
