...
#include <vast/postgres_plugin/postgres_plugin.hpp>
#endif
...
using namespace vast::chain::plugin_interface;

namespace {
bool
failure_is_subjective(const fc::exception& e, bool deadline_is_subjective) {
    auto code = e.code();
    return (code == deadline_exception::code_value && deadline_is_subjective);
}
}  // namespace
...
public:
    ...
    uint32_t                              _production_skip_flags = 0;  //vast::chain::skip_nothing;
    ...
    fc::microseconds _vastwd_provider_timeout_us;
    ...
    void
    on_block(const block_state_ptr& bsp) {
    ...
    void
    on_irreversible_block(const signed_block_ptr& lib) {
        _irreversible_block_time = lib->timestamp.to_time_point();
    }

    template <typename Type, typename Channel, typename F>
    auto
    publish_results_of(const Type& data, Channel& channel, F f) {
    ...
    void
    on_incoming_block(const signed_block_ptr& block) {
        auto id = block->id();

        fc_dlog(_log, "received incoming block ${id}", ("id", id));

        VAST_ASSERT(block->timestamp < (fc::time_point::now() + fc::seconds(7)), block_from_the_future,
    ...
    void
    on_incoming_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
        chain::controller& chain = chain_plug->chain();
        const auto&        cfg   = chain.get_global_properties().configuration;

        app().get_io_service().post([self = this, trx, persist_until_expired, next]() {
            self->process_incoming_transaction_async(trx, persist_until_expired, next);
        });
    }

    void
    process_incoming_transaction_async(const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) {
        chain::controller& chain = chain_plug->chain();
...
void
new_chain_banner(const vast::chain::controller& db) {
    std::cerr << "\n"
                 "************************************\n"
                 "*                                  *\n"
                 "*   --------- NEW CHAIN --------   *\n"
...
producer_plugin::set_program_options(
    ...
    producer_options.add_options()
        ("enable-stale-production,e", boost::program_options::bool_switch()->notifier(
            [this](bool e) { my->_production_enabled = e; }), "Enable block production, even if the chain is stale.")
        ...
        ("signature-provider", boost::program_options::value<vector<string>>()->composing()->multitoken()->default_value(
            ...
            "Where:\n"
            "   <public-key>    \tis a string form of a vaild VAST public key\n\n"
            "   <provider-spec> \tis a string in the form <provider-type>:<data>\n\n"
            "   <provider-type> \tis KEY, or VASTWD\n\n"
            "   KEY:<data>      \tis a string form of a valid VAST private key which maps to the provided public key\n\n"
            "   VASTWD:<data>    \tis the URL where vastwd is available and the approptiate wallet(s) are unlocked")
         ("vastwd-provider-timeout", boost::program_options::value<int32_t>()->default_value(5), "Limits the maximum time (in milliseconds) that is allowed for sending blocks to a vastwd provider for signing")
...
bool
producer_plugin::is_producer_key(const chain::public_key_type& key) const {
...
producer_plugin::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const {
    if(key != chain::public_key_type()) {
        auto private_key_itr = my->_signature_providers.find(key);
        VAST_ASSERT(private_key_itr != my->_signature_providers.end(), producer_priv_key_not_found, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
static producer_plugin_impl::signature_provider_type
make_vastwd_signature_provider(const std::shared_ptr<producer_plugin_impl>& impl, const string& url_str, const public_key_type pubkey) {
    auto vastwd_url = fc::url(url_str);
    std::weak_ptr<producer_plugin_impl> weak_impl = impl;

    return [weak_impl, vastwd_url, pubkey](const chain::digest_type& digest) {
        auto impl = weak_impl.lock();
        if(impl) {
            fc::variant params;
            fc::to_variant(std::make_pair(digest, pubkey), params);
            auto deadline = impl->_vastwd_provider_timeout_us.count() >= 0 ? fc::time_point::now() + impl->_vastwd_provider_timeout_us : fc::time_point::maximum();
            return app().get_plugin<http_client_plugin>().get_client().post_sync(vastwd_url, params, deadline).as<chain::signature_type>();
        }
        else {
...
producer_plugin::plugin_initialize(const boost::program_options::variables_map& options) {
    ...
    try {
        ...
        VAST_ASSERT(my->chain_plug, plugin_config_exception, "chain_plugin not found");
        ...
        if(options.count("signature-provider")) {
            ...
            for(const auto& key_spec_pair : key_spec_pairs) {
                try {
                    ...
                    VAST_ASSERT(delim != std::string::npos, plugin_config_exception, "Missing \"=\" in the key spec pair");
                    auto pub_key_str = key_spec_pair.substr(0, delim);
                    auto spec_str    = key_spec_pair.substr(delim + 1);

                    auto spec_delim = spec_str.find(":");
                    VAST_ASSERT(spec_delim != std::string::npos, plugin_config_exception, "Missing \":\" in the key spec pair");
                    ...
                    else if(spec_type_str == "VASTWD") {
                        my->_signature_providers[pubkey] = make_vastwd_signature_provider(my, spec_data, pubkey);
                    }
                }
                catch(...) {
        ...
        my->_vastwd_provider_timeout_us = fc::milliseconds(options.at("vastwd-provider-timeout").as<int32_t>());
        ...
        if(options.count("snapshots-dir")) {
            ...
            VAST_ASSERT(fc::is_directory(my->_snapshots_dir), snapshot_directory_not_found_exception,
        ...
        my->_incoming_transaction_async_provider = app().get_method<incoming::methods::transaction_async>().register_provider(
            [this](const transaction_metadata_ptr& trx, bool persist_until_expired, next_function<transaction_trace_ptr> next) -> void {
...
producer_plugin::plugin_startup() {
    try {
        ...
        VAST_ASSERT(my->_producers.empty() || chain.get_read_mode() == chain::db_read_mode::SPECULATIVE, plugin_config_exception,
            "node cannot have any producer-name configured because block production is impossible when read_mode is not \"speculative\"");

        VAST_ASSERT(my->_producers.empty() || chain.get_validation_mode() == chain::validation_mode::FULL, plugin_config_exception,
        ...
        if(!my->_producers.empty()) {
            ...
            if(my->_production_enabled) {
                ...
                //_production_skip_flags |= vast::chain::skip_undo_history_check;
            }
        }
...
void
producer_plugin::plugin_shutdown() {
    try {
        my->_timer.cancel();
    }
    catch(fc::exception& e) {
...
producer_plugin::get_integrity_hash() const {
    chain::controller& chain = my->chain_plug->chain();
...
producer_plugin::create_snapshot(const create_snapshot_options& options) const {
    ...
    VAST_ASSERT(!fc::is_regular_file(snapshot_path), snapshot_exists_exception,
    ...
    if(options.postgres) {
#ifdef POSTGRES_SUPPORT
        if(app().find_plugin("vast::postgres_plugin") == nullptr) {
...
producer_plugin_impl::calculate_next_block_time(const account_name& producer_name, const block_timestamp_type& current_block_time) const {
    chain::controller& chain           = chain_plug->chain();
...
fc::time_point
producer_plugin_impl::calculate_pending_block_time() const {
...
fc::time_point
producer_plugin_impl::calculate_block_deadline(const fc::time_point& block_time) const {
...
producer_plugin_impl::schedule_production_loop() {
    ...
    if(result == start_block_result::failed) {
        ...
        if(deadline > fc::time_point::now()) {
            ...
            VAST_ASSERT(chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state, start_block succeeded");
            ...
            VAST_ASSERT(chain.pending_block_state(), missing_pending_block_state, "producing without pending_block_state");
        ...
        VAST_ASSERT(chain.pending_block_state(), missing_pending_block_state, "speculating without pending_block_state");
        const auto& pbs = chain.pending_block_state();
...
producer_plugin_impl::produce_block() {
    VAST_ASSERT(_pending_block_mode == pending_block_mode::producing, producer_exception, "called produce_block while not actually producing");

    chain::controller& chain = chain_plug->chain();
    const auto&        pbs   = chain.pending_block_state();
    const auto&        hbs   = chain.head_block_state();
    VAST_ASSERT(pbs, missing_pending_block_state, "pending_block_state does not exist but it should, another plugin may have corrupted it");
    auto signature_provider_itr = _signature_providers.find(pbs->block_signing_key);

    VAST_ASSERT(signature_provider_itr != _signature_providers.end(), producer_priv_key_not_found, "Attempting to produce a block for which we don't have the private key");
