...
#include <vast/bnet_plugin/bnet_plugin.hpp>
#include <vast/chain/controller.hpp>
#include <vast/chain/trace.hpp>
...
using vast::block_id_type;
using vast::block_timestamp_type;
using vast::chain_id_type;
using vast::packed_transaction_ptr;
using vast::public_key_type;
using vast::signed_block_ptr;
using std::string;
using std::vector;
using fc::sha256;
...
public:
    ...
    struct block_status {
        ...
        uint32_t
        block_num() const { return block_header::num_from_id(id); }
    };
    ...
    struct transaction_status {
        ...
        void
        mark_known_by_peer() {
            received = fc::time_point::maximum();
            trx.reset();
        }
        bool
        known_by_peer() const { return received == fc::time_point::maximum(); }
    };
    ...
    void
    on_resolve(boost::system::error_code   ec,
    ...
    /**
         ...
         */
    void
    on_accepted_transaction(transaction_metadata_ptr t) {
    ...
    template <typename L>
    void
    async_get_pending_block_ids(L&& callback) {
    ...
    template <typename L>
    void
    async_get_block_num(uint32_t blocknum, L&& callback) {
    ...
    void
    mark_block_status( const block_id_type& id, bool known_by_peer, bool recv_from_peer ) {
        auto itr = _block_status.find(id);
        if(itr == _block_status.end()) {
    ...
    void
    mark_block_recv_from_peer(block_id_type id) {
        auto itr = _block_status.find(id);
        if(itr == _block_status.end()) {
    ...
    maybe_send_next_message() {
        ...
        if(_out_buffer.size())
            return;  /// in process of sending
        if(!_recv_remote_hello || !_sent_remote_hello)
            return;

        clear_expired_trx();

        if(send_block_notice())
            return;
        if(send_pong())
            return;
        if(send_ping())
    ...
    bool
    is_known_by_peer(block_id_type id) {
        auto itr = _block_status.find(id);
        if(itr == _block_status.end())
            return false;
    ...
    void
    do_goodbye(const string& reason) {
        try {
    ...
    on(const signed_block_ptr& b) {
        ...
        if(!b) {
            ...
            VAST_THROW(block_validate_exception, "bad block");
        }
    ...
    /**
         ...
         */
    bool
    mark_transaction_known_by_peer(const transaction_id_type& id) {
...
public:
    ...
    void
    run() {
        VAST_ASSERT(_acceptor.is_open(), plugin_exception, "unable top open listen socket");
        do_accept();
    }
...
public:
    ...
    void
    async_add_session(std::weak_ptr<session> wp) {
    ...
    */
    ...
    void
    on_accepted_block_header(block_state_ptr s) {
        _ioc->post([s, this] {  /// post this to the thread pool because packing can be intensive
    ...
    /**
          ...
          */
    void
    on_bad_block(signed_block_ptr s) {
...
bnet_plugin::plugin_startup() {
    ...
    my->_on_bad_block_handle = app().get_channel<channels::rejected_block>().subscribe([this](signed_block_ptr b) {
        my->on_bad_block(b);
    });
...
bnet_plugin::plugin_shutdown() {
    ...
    my->for_each_session([](auto ses){
        VAST_ASSERT(false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num));
    });
...
void
session::check_for_redundant_connection() {
...
session::on(const packed_transaction_ptr& p) {
    ...
    if(!p) {
        ...
        VAST_THROW(transaction_exception, "bad transaction");
    }
