...
#include <vast/http_plugin/local_endpoint.hpp>
...
public:
    ...
    on_tls_init(websocketpp::connection_hdl hdl) {
        ...
        try {
            ...
            if(!ecdh)
                VAST_THROW(chain::http_exception, "Failed to set NID_secp384r1");
            if(SSL_CTX_set_tmp_ecdh(ctx->native_handle(), (EC_KEY*)ecdh) != 1)
                VAST_THROW(chain::http_exception, "Failed to set ECDH PFS");
            ...
            if(SSL_CTX_set_cipher_list(ctx->native_handle(),
               ...
               != 1)
                VAST_THROW(chain::http_exception, "Failed to set HTTPS cipher list");
        }
    ...
    alloc_deferred_id(typename websocketpp::server<T>::connection_ptr con) {
        if(http_conn_count + https_conn_count >= max_deferred_connection_size) {
            VAST_THROW2(chain::exceed_deferred_request, "Exceed max allowed deferred connections, max: {}", max_deferred_connection_size);
        }
        ...
        VAST_THROW2(chain::alloc_deferred_fail,
