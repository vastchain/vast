 ...
 */
#include <vast/trafficgen_plugin/trafficgen_plugin.hpp>
...
using vast::chain::action;
using vast::chain::block_id_type;
using vast::chain::block_state_ptr;
using vast::chain::packed_transaction_ptr;
using vast::chain::private_key_type;
using vast::chain::transaction_metadata;
...
trafficgen_plugin::plugin_initialize(const variables_map& options) {
    ...
    VAST_ASSERT(my_->total_num_ <= 200'000, chain::plugin_config_exception, "Total number of generating transactions cannot be large than 200'000");
    ...
    if(options.count("traffic-type")) {
        ...
        VAST_ASSERT(type == "ft" || type == "nft", chain::plugin_config_exception, "Not valid value for --traffic-type option");
        my_->type_ = type;
    }
}
