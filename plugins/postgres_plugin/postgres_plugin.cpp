...
#include <vast/utilities/spinlock.hpp>
...
using vast::utilities::spinlock;
using vast::utilities::spinlock_guard;
...
public:
    postgres_plugin_impl(const controller& control)
        : control_(control)
        , exec_ctx_(dynamic_cast<const vast_execution_context&>(control.get_execution_context())) {}
...
postgres_plugin_impl::applied_irreversible_block(const block_state_ptr& bsp) {
    vast::internal::queueb(block_state_queue_, std::make_tuple(bsp, true), lock_, cond_, queue_size_);
}

void
postgres_plugin_impl::applied_block(const block_state_ptr& bsp) {
    vast::internal::queueb(block_state_queue_, std::make_tuple(bsp, false), lock_, cond_, queue_size_);
}
...
postgres_plugin_impl::applied_transaction(const transaction_trace_ptr& ttp) {
    ...
    vast::internal::queuet(transaction_trace_queue_, ttp, lock_, cond_);
}

void
postgres_plugin_impl::consume_queues() {
    using namespace vast::internal;
...
postgres_plugin_impl::verify_last_block(const std::string& prev_block_id) {
    ...
    if(!db_.get_latest_block_id(last_block_id)) {
        VAST_THROW(postgres_plugin_exception, "No blocks found in database");
    }

    VAST_ASSERT(prev_block_id == last_block_id, postgres_plugin_exception,
...
postgres_plugin_impl::verify_no_blocks() {
    if(!db_.is_table_empty("blocks")) {
        VAST_THROW(postgres_plugin_exception, "Existing blocks found in database");
    }
}
...
postgres_plugin_impl::_process_block(const block_state_ptr block, std::deque<transaction_trace_ptr>& traces, copy_context& cctx, trx_context& tctx) {
    ...
    if(block->block_num <= last_sync_block_num_) {
        VAST_ASSERT(db_.exists_block(id), postgres_sync_exception,
...
postgres_plugin_impl::init(bool init_db) {
    if(!init_db) {
        ...
        VAST_RETHROW_EXCEPTIONS(vast::postgres_plugin_exception,
    ...
    if(init_db) {
        ...
        // HACK: Add VAST and PVAST manually
        auto tctx = db_.new_trx_context();
        tctx.set_trx_num(0);

        auto gs = chain::genesis_state();
        db_.add_fungible(tctx, gs.get_vast_ft());
        db_.add_fungible(tctx, gs.get_pvast_ft());

        auto ng  = newgroup();
        ng.name  = N128(.everiToken);
        ng.group = gs.vast_org;
...
postgres_plugin::set_program_options(options_description& cli, options_description& cfg) {
    cfg.add_options()
        ("postgres-queue-size,q", bpo::value<uint>()->default_value(5120), "The queue size between vastd and postgres plugin thread.")
...
postgres_plugin::plugin_initialize(const variables_map& options) {
    ...
    if(options.count("postgres-uri")) {
        ...
        if(options.at("clear-postgres").as<bool>()) {
            ...
            VAST_ASSERT(delete_state, postgres_plugin_exception,
        ...
        if(options.count("snapshot")) {
            ...
            VAST_ASSERT(fc::exists(snapshot_path), plugin_config_exception,
            ...
            if(reader->has_section("pg-blocks")) {
                VAST_ASSERT(delete_state, postgres_plugin_exception, "Snapshot can only be used to initialize an empty database, please use --delete-all-blcoks or --clear-postgres option.");
        ...
        wlog("vast::postgres_plugin configured, but no --postgres-uri specified.");
