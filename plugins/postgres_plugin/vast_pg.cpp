...
#include <vast/chain/block_header.hpp>
#include <vast/chain/exceptions.hpp>
#include <vast/chain/snapshot.hpp>
#include <vast/chain/contracts/abi_serializer.hpp>
#include <vast/postgres_plugin/copy_context.hpp>
#include <vast/postgres_plugin/trx_context.hpp>
...
pg::connect(const std::string& conn) {
    ...
    VAST_ASSERT(status == CONNECTION_OK, chain::postgres_connection_exception, "Connect failed");
...
pg::init_pathman() {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_COMMAND_OK, chain::postgres_exec_exception, "Setup extension pg_pathman failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
    PQclear(r);
    return PG_OK;
}
...
pg::create_partitions(const std::string& table, const std::string& relation, uint interval, uint part_nums) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Create partitions failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
    PQclear(r);
    return PG_OK;
}
...
pg::drop_partitions(const std::string& table) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Drop partitions failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
    PQclear(r);
    return PG_OK;
}
...
pg::create_db(const std::string& db) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_COMMAND_OK, chain::postgres_exec_exception, "Create database failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::drop_db(const std::string& db) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_COMMAND_OK, chain::postgres_exec_exception, "Drop database failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::exists_db(const std::string& db) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Check if database existed failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::exists_table(const std::string& table) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Check if table existed failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::is_table_empty(const std::string& table) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Get one block id failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::drop_table(const std::string& table) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_COMMAND_OK, chain::postgres_exec_exception, "Drop table failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::drop_sequence(const std::string& seq) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_COMMAND_OK, chain::postgres_exec_exception, "Drop sequence failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::prepare_tables() {
    ...
    for(auto stmt : stmts) {
        ...
        VAST_ASSERT(PQresultStatus(r) == PGRES_COMMAND_OK, chain::postgres_exec_exception,
...
pg::check_version() {
    ...
    if(!read_stat("version", cur_ver)) {
        VAST_THROW(chain::postgres_version_exception, "Version information doesn't exist in current database");
    }
    VAST_ASSERT(cur_ver >= pg_version, chain::postgres_version_exception, "Version of current postgres database is obsolete, cur: ${c}, latest: ${l}", ("c",cur_ver)("l",pg_version));
    return PG_OK;
}
...
pg::check_last_sync_block() {
    ...
    if(!read_stat("last_sync_block_id", sync_block_id)) {
        VAST_THROW(chain::postgres_sync_exception, "Last sync block id doesn't exist in current database");
    }
    auto last_block_id = std::string();
    if(get_latest_block_id(last_block_id)) {
        VAST_ASSERT(sync_block_id == last_block_id, chain::postgres_sync_exception, "Sync block and latest block are not match, sync is ${s}, latest is ${l}", ("s",sync_block_id)("l",last_block_id));
        last_sync_block_id_ = last_block_id;
        return PG_OK;
    }
    VAST_THROW(chain::postgres_sync_exception, "Cannot get latest block id");
}
...
pg::block_copy_to(const std::string& table, const std::string& data) {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_COPY_IN, chain::postgres_exec_exception, "Not expected COPY response, detail: ${s}", ("s",PQerrorMessage(conn_)));
    PQclear(r);

    auto nr = PQputCopyData(conn_, data.data(), (int)data.size());
    VAST_ASSERT(nr == 1, chain::postgres_exec_exception, "Put data into COPY stream failed, detail: ${s}", ("s",PQerrorMessage(conn_)));

    auto nr2 = PQputCopyEnd(conn_, NULL);
    VAST_ASSERT(nr2 == 1, chain::postgres_exec_exception, "Close data into COPY stream failed, detail: ${s}", ("s",PQerrorMessage(conn_)));

    auto r2 = PQgetResult(conn_);
    VAST_ASSERT(PQresultStatus(r2) == PGRES_COMMAND_OK, chain::postgres_exec_exception, "Execute COPY command failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
    PQclear(r2);
...
pg::commit_trx_context(trx_context& tctx) {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_COMMAND_OK, chain::postgres_exec_exception, "Commit transactions failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::get_latest_block_id(std::string& block_id) const {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Get latest block id failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::get_latest_trx_num(int64_t& trx_num) const {
    ...
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Get latest trx num failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::exists_block(const std::string& block_id) const {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Check block existed failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
pg::read_stat(const std::string& key, std::string& value) const {
    ...
    auto r = PQexec(conn_, stmt.c_str());
    VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK, chain::postgres_exec_exception, "Get stat value failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
...
PREPARE_SQL_ONCE(dt_plan, "UPDATE tokens SET owner = '{\"VAST00000000000000000000000000000000000000000000000000\"}' WHERE id = $1;");
...
pg::add_fungible(trx_context& tctx, const fungible_def& ft) {
    ...
    if(!ft.metas.empty()) {
        for(auto& m : ft.metas) {
            ...
            auto act = vast::chain::action(N128(.fungible), vast::chain::name128::from_number(ft.sym.id()), am);
            add_meta(tctx, act);
        }
    }
...
pg::backup(const std::shared_ptr<chain::snapshot_writer>& snapshot) const {
    ...
    for(const auto& t : tables) {
        ...
        snapshot->write_section(fmt::format("pg-{}", t.name), [this, &t](auto& writer) {
            ...
            auto r = PQexec(conn_, stmt.c_str());
            VAST_ASSERT(PQresultStatus(r) == PGRES_COPY_OUT, chain::postgres_exec_exception, "Not expected COPY response, detail: ${s}", ("s",PQerrorMessage(conn_)));
            PQclear(r);
            ...
            if(cr == -2) {
                VAST_THROW(chain::postgres_exec_exception, "COPY OUT table failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
            }
            VAST_ASSERT(cr == -1, chain::postgres_exec_exception, "Not expected COPY response, detail: ${s}", ("s",PQerrorMessage(conn_)));
    ...
    for(const auto& s : sequences) {
        ...
        snapshot->write_section(fmt::format("pg-seq-{}", s.name), [this, &s](auto& writer) {
            ...
            auto r = PQexec(conn_, stmt.c_str());
            VAST_ASSERT(PQresultStatus(r) == PGRES_TUPLES_OK && PQntuples(r) == 1, chain::postgres_exec_exception, "Get latest sequence value failed, detail: ${s}", ("s",PQerrorMessage(conn_)));
