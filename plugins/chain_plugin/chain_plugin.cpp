...
#include <vast/utilities/key_conversion.hpp>
...
std::ostream&
operator<<(std::ostream& osm, vast::chain::db_read_mode m) {
    if(m == vast::chain::db_read_mode::SPECULATIVE) {
        osm << "speculative";
    }
    else if(m == vast::chain::db_read_mode::HEAD) {
        osm << "head";
    }
    else if(m == vast::chain::db_read_mode::READ_ONLY) {
        osm << "read-only";
    }
    else if(m == vast::chain::db_read_mode::IRREVERSIBLE) {
        osm << "irreversible";
    }
...
validate(boost::any&                     v,
         ...
         vast::chain::db_read_mode* /* target_type */,
         int) {
    ...
    if(s == "speculative") {
        v = boost::any(vast::chain::db_read_mode::SPECULATIVE);
    }
    else if(s == "head") {
        v = boost::any(vast::chain::db_read_mode::HEAD);
    }
    else if(s == "read-only") {
        v = boost::any(vast::chain::db_read_mode::READ_ONLY);
    }
    else if(s == "irreversible") {
        v = boost::any(vast::chain::db_read_mode::IRREVERSIBLE);
    }
    else {
...
std::ostream&
operator<<(std::ostream& osm, vast::chain::validation_mode m) {
    if(m == vast::chain::validation_mode::FULL) {
        osm << "full";
    }
    else if(m == vast::chain::validation_mode::LIGHT) {
        osm << "light";
    }
...
validate(boost::any&                     v,
         const std::vector<std::string>& values,
         vast::chain::validation_mode* /* target_type */,
         int) {
    using namespace boost::program_options;
    ...
    if(s == "full") {
        v = boost::any(vast::chain::validation_mode::FULL);
    }
    else if(s == "light") {
        v = boost::any(vast::chain::validation_mode::LIGHT);
    }
    else {
        throw validation_error(validation_error::invalid_option_value);
    }
...
std::ostream&
operator<<(std::ostream& osm, vast::chain::storage_profile m) {
    if(m == vast::chain::storage_profile::disk) {
        osm << "disk";
    }
    else if(m == vast::chain::storage_profile::memory) {
        osm << "memory";
    }
...
validate(boost::any&                     v,
         const std::vector<std::string>& values,
         vast::chain::storage_profile* /* target_type */,
         int) {
    using namespace boost::program_options;
    ...
    if(s == "disk") {
        v = boost::any(vast::chain::storage_profile::disk);
    }
    else if(s == "memory") {
        v = boost::any(vast::chain::storage_profile::memory);
    }
    else {
        throw validation_error(validation_error::invalid_option_value);
    }
...
using namespace vast::chain::config;
...
chain_plugin::chain_plugin()
    ...
    app().register_config_type<vast::chain::db_read_mode>();
    app().register_config_type<vast::chain::validation_mode>();
    app().register_config_type<vast::chain::storage_profile>();
}
...
chain_plugin::set_program_options(options_description& cli, options_description& cfg) {
    cfg.add_options()
        ...
        ("token-db-profile", boost::program_options::value<vast::chain::storage_profile>()->default_value(vast::chain::storage_profile::disk),
        ...
        ("read-mode", boost::program_options::value<vast::chain::db_read_mode>()->default_value(vast::chain::db_read_mode::SPECULATIVE),
        ...
        ("validation-mode", boost::program_options::value<vast::chain::validation_mode>()->default_value(vast::chain::validation_mode::FULL),
...
chain_plugin::plugin_initialize(const variables_map& options) {
    ...
    try {
        try {
            genesis_state gs;  // Check if VAST_ROOT_KEY is bad
        }
        catch(const fc::exception&) {
            elog("VAST_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.", ("root_key", genesis_state::vast_root_key));
            throw;
        }
        ...
        if(options.count("checkpoint")) {
            ...
            for(const auto& cp : cps) {
                ...
                if(itr != my->loaded_checkpoints.end()) {
                    VAST_ASSERT(itr->second == item.second, plugin_config_exception,
        ...
        if(options.count("extract-genesis-json") || options.at("print-genesis-json").as<bool>()) {
            ...
            VAST_THROW(extract_genesis_state_exception, "extracted genesis state from blocks.log");
        }
        ...
        if(options.count("export-reversible-blocks")) {
            ...
            VAST_THROW(node_management_success, "exported reversible blocks");
        }
        ...
        if(options.at("delete-all-blocks").as<bool>()) {
            ...
            VAST_THROW(fixed_reversible_db_exception, "fixed corrupted reversible blocks database");
        }
            ...
            VAST_THROW(node_management_success, "imported reversible blocks");
        }
        ...
        if(options.count("snapshot")) {
            ...
            VAST_ASSERT(fc::exists(*my->snapshot_path), plugin_config_exception,
            ...
            VAST_ASSERT(options.count("genesis-json") == 0 && options.count("genesis-timestamp") == 0,
            ...
            VAST_ASSERT(!fc::exists(shared_mem_path),
                       plugin_config_exception,
            ...
            if(fc::is_regular_file(my->blocks_dir / "blocks.log")) {
                ...
                VAST_ASSERT(log_genesis.compute_chain_id() == my->chain_config->genesis.compute_chain_id(),
                           plugin_config_exception,
            ...
            if(options.count("genesis-json")) {
                ...
                VAST_ASSERT(!fc::exists(my->blocks_dir / "blocks.log"),
                           plugin_config_exception,
                ...
                VAST_ASSERT(fc::is_regular_file(genesis_file),
                           plugin_config_exception,
            ...
            if(!existing_genesis) {
                ...
                VAST_ASSERT(my->chain_config->genesis == *existing_genesis, plugin_config_exception,
        ...
        if(options.count("read-mode")) {
            ...
            VAST_ASSERT(my->chain_config->read_mode != db_read_mode::IRREVERSIBLE, plugin_config_exception, "irreversible mode not currently supported.");
        }
        ...
        my->pre_accepted_block_connection = my->chain->pre_accepted_block.connect([this](const signed_block_ptr& blk) {
            ...
            if(itr != my->loaded_checkpoints.end()) {
                auto id = blk->id();
                VAST_ASSERT(itr->second == id, checkpoint_exception,
...
chain_plugin::recover_reversible_blocks(const fc::path& db_dir, uint32_t cache_size,
    ...
    if(new_db_dir) {
        ...
        VAST_ASSERT(reversible_dir_name != ".", invalid_reversible_blocks_dir, "Invalid path to reversible directory");
        backup_dir = reversible_dir.parent_path() / reversible_dir_name.append("-").append(now);

        VAST_ASSERT(!fc::exists(backup_dir),
    ...
    try {
        for(; itr != ubi.end(); ++itr) {
            VAST_ASSERT(itr->blocknum == end + 1, gap_in_reversible_blocks_db,
    ...
    try {
        while(reversible_blocks.tellg() < end_pos) {
            ...
            if(start == 0) {
                ...
                VAST_ASSERT(num == end + 1, gap_in_reversible_blocks_db,
...
chain_plugin::get_chain_id() const {
    VAST_ASSERT(my->chain_id.has_value(), chain_id_type_exception, "Chain ID has not been initialized yet");
    return *my->chain_id;
}
...
read_only::get_info(const read_only::get_info_params&) const {
    ...
    return {
        ...
        db.get_chain_id(),
        contracts::vast_contract_abi_version(),
...
read_only::get_block(const read_only::get_block_params& params) const {
    ...
    VAST_ASSERT(!params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
    ...
    VAST_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${block_num_or_id}", ("block_num_or_id", params.block_num_or_id))

    VAST_ASSERT(block, unknown_block_exception, "Could not find block: ${block}", ("block", params.block_num_or_id));
...
read_only::get_block_header_state(const get_block_header_state_params& params) const {
    ...
    VAST_ASSERT(b, unknown_block_exception, "Could not find reversible block: ${block}", ("block", params.block_num_or_id));
...
read_only::get_head_block_header_state(const get_head_block_header_state_params& params) const {
    ...
    VAST_ASSERT(b, unknown_block_exception, "Could not find head block");
...
read_only::get_transaction(const get_transaction_params& params) {
    ...
    VAST_ASSERT(block, unknown_block_exception, "Could not find head block");
    ...
    VAST_THROW(unknown_transaction_exception, "Cannot find transaction");
}
...
read_only::get_trx_id_for_link_id(const get_trx_id_for_link_id_params& params) const {
    if(params.link_id.size() != sizeof(link_id_type)) {
        VAST_THROW(vast_link_id_exception, "VAST-Link id is not in proper length");
    }
...
read_write::push_transaction(const read_write::push_transaction_params& params, next_function<read_write::push_transaction_results> next) {
    try {
        ...
        VAST_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid packed transaction")
...
read_only::abi_json_to_bin(const read_only::abi_json_to_bin_params& params) const try {
    ...
    VAST_RETHROW_EXCEPTIONS(chain::action_args_exception,
...
read_only::trx_json_to_digest(const trx_json_to_digest_params& params) const {
    ...
    try {
        ...
        VAST_RETHROW_EXCEPTIONS(chain::packed_transaction_type_exception, "Invalid transaction");
...
read_only::get_required_keys(const get_required_keys_params& params) const {
    ...
    VAST_RETHROW_EXCEPTIONS(chain::transaction_type_exception, "Invalid transaction");
...
read_only::get_transaction_ids_for_block(const get_transaction_ids_for_block_params& params) const {
    ...
    VAST_RETHROW_EXCEPTIONS(chain::block_id_type_exception, "Invalid block ID: ${id}", ("id", params.block_id))

    VAST_ASSERT(block, unknown_block_exception, "Could not find block: ${block}", ("id", params.block_id));
...
read_only::get_abi(const get_abi_params&) const {
    ...
    if(_abi_json.empty()) {
        auto abi = vast::chain::contracts::vast_contract_abi();
