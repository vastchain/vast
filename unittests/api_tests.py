...
import click
import grequests
from pyvast import abi, ecc, vast_link, libvast
from pyvastsdk import action, api, base, transaction, unit_test
    ...
    "rules": [
        {
            ...
            "data": {
                "receiver": {
                    "type": "address",
                    "data": "VAST6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV"
                },
                "amount": "10.00000 S#3"
            }
        },
...
lock_json_raw = '''
{
        ...
        "assets": [{
            "type": "fungible",
            "data": {
                "from": "VAST7rbe5ZqAEtwQT6Tw39R29vojFqrCQasK3nT5s2pEzXh1BABXHF",
                "amount": "5.00000 S#3"
            }
        }],
        "condition": {
...
def pre_action():
    prodvote = AG.new_action(
        'prodvote', producer='vast', key="action-everipay", value=2)

    newdomain = AG.new_action(
        'newdomain', name=domain_name, creator=user.pub_key)

    auth_A = base.AuthorizerWeight(base.AuthorizerRef('A', vast_pub), 1)
    ...
    pass_link = vast_link.VastLink()
    pass_link.set_header(vast_link.HeaderType.version1.value |
                         vast_link.HeaderType.everiPass.value | vast_link.HeaderType.destroy.value)
    ...
    issuefungible2 = AG.new_action(
        'issuefungible', address=base.Address().set_public_key(user.pub_key), number=base.VastAsset(1000), memo='goodluck')

    e2p = AG.new_action(
        'vast2pvast', _from=base.Address().set_public_key(user.pub_key), to=base.Address().set_public_key(user.pub_key), number=base.VastAsset(100), memo='goodluck')

    pay_link = vast_link.VastLink()
    pay_link.set_timestamp(int(time.time()))
    pay_link.set_max_pay(999999999)
    pay_link.set_header(vast_link.HeaderType.version1.value |
                        vast_link.HeaderType.everiPay.value)
...
class Test(unittest.TestCase):
    ...
    def _test_vast_link_response(self, resp):
        j = json.loads(resp)
    ...
    def test_vast_link_for_trx_id(self):
        symbol = base.Symbol(
            sym_name=sym_name, sym_id=sym_id, precision=sym_prec)
        asset = base.new_asset(symbol)
        ...
        for i in range(1000):
            ...
            self._test_vast_link_response(resp)

    def test_vast_link_for_trx_id2(self):
        symbol = base.Symbol(
            sym_name=sym_name, sym_id=sym_id, precision=sym_prec)
        asset = base.new_asset(symbol)
    ...
    def test_vast_link_for_trx_id3(self):
        symbol = base.Symbol(
            sym_name=sym_name, sym_id=sym_id, precision=sym_prec)
        asset = base.new_asset(symbol)
        pay_link = vast_link.VastLink()
        ...
        url = 'http://127.0.0.1:8888/v1/vast_link/get_trx_id_for_link_id'
    ...
    def test_vast_link_for_trx_id4(self):
        symbol = base.Symbol(
            sym_name=sym_name, sym_id=sym_id, precision=sym_prec)
        asset = base.new_asset(symbol)
        pay_link = vast_link.VastLink()
    ...
    def test_get_fungible_actions(self):
        req = {
            'sym_id': 338422621,
            'address': 'VAST6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV',
            'dire': 'asc',
            'skip': 0,
            'take': 10
        }
        ...
        self.assertTrue('data' in res_dict[0].keys(), msg=resp)
        self.assertTrue('timestamp' in res_dict[0].keys(), msg=resp)
        self.assertTrue('vast2pvast' in resp, msg=resp)

    def test_get_history_transaction(self):
        prodvote = AG.new_action(
        'prodvote', producer='vast', key="global-charge-factor", value=10000)
...
@click.option('--start-vastd', '-s', default=True)
@click.option('--vastd-path', '-m', default='/home/laighno/Documents/vast/programs/vastd/vastd')
@click.option('--public-key', '-p', type=str, default='VAST8CAme1QR2664bLQsVfrERBkXL3xEKsALMSfogGavaXFkaVVqR1')
@click.option('--private-key', '-k', type=str, default='5JFZQ7bRRuBJyh42uV5ELwswr2Bt3rfwmyEsyXAoUue18NUreAF')
def main(url, start_vastd, vastd_path, public_key, private_key):
    global vastdout
    vastdout = None

    p = None
    if start_vastd == True:
        vastdout = open('/tmp/vast_api_tests_vastd.log', 'w')

        p = subprocess.Popen([vastd_path, '-e', '--http-validate-host=false',  '--plugin=vast::postgres_plugin',
                              '--plugin=vast::history_plugin', '--plugin=vast::history_api_plugin', '--plugin=vast::chain_api_plugin', '--plugin=vast::vast_api_plugin',
                              '--plugin=vast::vast_link_plugin', '--producer-name=vast', '--delete-all-blocks', '-d', './tmp', '--postgres-uri=postgresql://postgres@localhost:5432/vast'],
                             stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=vastdout, shell=False)
        
        # wait for vastd to initialize
        time.sleep(3)
    ...
    try:
        ...
        global vast_pub, priv_vast
        vast_pub = ecc.PublicKey.from_string(
        ...
        global api, VastAsset, AG
        api = api.Api(url)
        VastAsset = base.VastAsset
