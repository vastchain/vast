...
TEST_CASE("test_address", "[types]") {
    ...
    CHECK(var1.get_string() == "VAST00000000000000000000000000000000000000000000000000");
    ...
    auto keystr = std::string("VAST6bMPrzVm77XSjrTfZxEsbAuWPuJ9hCqGRLEhkTjANWuvWTbwe3");
    ...
    CHECK(var2.get_string() == "VAST6bMPrzVm77XSjrTfZxEsbAuWPuJ9hCqGRLEhkTjANWuvWTbwe3");
...
TEST_CASE("test_link_1", "[types]") {
    ...
    auto link = vast_link::parse_from_vastli(str);
...
TEST_CASE("test_link_2", "[types]") {
    auto str = "https://vast.li/04OH4QSYU-9:0ISOMCF2AY*JO/O/7VTMZLC6W*F0NQ831F+60"
    ...
    CHECK(*link.get_segment(vast_link::timestamp).intv == 1532465608);
    CHECK(link.get_segment(vast_link::domain).intv.has_value() == false);
    CHECK(*link.get_segment(vast_link::domain).strv == "testdomain");
    CHECK(*link.get_segment(vast_link::token).strv == "testtoken");
...
TEST_CASE("test_link_3", "[types]") {
    ...
    // CHECK(str == str2); because vastjs generated link doesn't guarantee the order of signatures

    CHECK(link.get_header() == 11);
    CHECK(*link.get_segment(vast_link::timestamp).intv == 1532468461);
    CHECK(link.get_segment(vast_link::domain).intv.has_value() == false);
    CHECK(*link.get_segment(vast_link::domain).strv == "testdomain");
    CHECK(*link.get_segment(vast_link::token).strv == "testtoken");
    ...
    CHECK(pkeys.find(public_key_type(std::string("VAST8HdQYD1xfKyD7Hyu2fpBUneamLMBXmP3qsYX6HoTw7yonpjWyC"))) != pkeys.end());
    CHECK(pkeys.find(public_key_type(std::string("VAST6MYSkiBHNDLxE6JfTmSA1FxwZCgBnBYvCo7snSQEQ2ySBtpC6s"))) != pkeys.end());
    CHECK(pkeys.find(public_key_type(std::string("VAST7bUYEdpHiKcKT9Yi794MiwKzx5tGY3cHSh4DoCrL4B2LRjRgnt"))) != pkeys.end());
}
...
TEST_CASE("test_symbol", "[types]") {
    ...
    CHECK(vast_sym() == symbol(5, 1));
    CHECK(pvast_sym() == symbol(5, 2));
}
...
TEST_CASE("test_percent_slim", "[types]") {
    ...
    auto CHECK_PARSE = [&](auto str, auto amount) {
        ...
        CHECK(a.raw_value() == (uint32_t)amount);
    };
    ...
    CHECK_THROWS_AS(percent_slim::from_string("111.00"), percent_type_exception);
    CHECK_THROWS_AS(percent_slim::from_string("111"), percent_type_exception);
    CHECK_THROWS_AS(percent_slim::from_string("1.1"), percent_type_exception);
