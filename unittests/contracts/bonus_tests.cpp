...
TEST_CASE_METHOD(contracts_test, "passive_bonus_test", "[contracts]") {
    ...
    auto keyseeds = std::vector<name>{ N(vast), N(key2), N(payer) };
    ...
    // vast cannot be used to set passive bonus
    actkey = name128::from_number(VAST_SYM_ID);
    ...
    rule3.receiver = dist_stack_receiver(asset(-1'00000, vast_sym()));
    ...
    CHECK(pb.rate.value() == vast::chain::percent_type("0.15"));
}
...
TEST_CASE_METHOD(contracts_test, "passive_bonus_fees_test", "[contracts]") {
    ...
    link.add_segment(vast_link::segment(vast_link::max_pay, 500'00000));
    link.add_segment(vast_link::segment(vast_link::symbol_id, get_sym_id()));
    link.add_segment(vast_link::segment(vast_link::link_id, "KIJHNHFMJDUKJUAA"));
    ...
    my_tester->add_money(tester::get_public_key(N(to4)), asset(10'00000, vast_sym()));
