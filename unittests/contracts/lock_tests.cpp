...
TEST_CASE_METHOD(contracts_test, "newnftlock_test", "[contracts]") {
    ...
    nl.succeed = {public_key_type(std::string("VAST8HdQYD1xfKyD7Hyu2fpBUneamLMBXmP3qsYX6HoTw7yonpjWyC"))};
    to_variant(nl, var);
...
TEST_CASE_METHOD(contracts_test, "tryunlock_test", "[contracts]") {
    ...
    READ_TOKEN2(token, get_domain_name(), "t3", tk);
    CHECK(tk.owner.size() == 1);
    CHECK(tk.owner[0] == public_key_type(std::string("VAST8HdQYD1xfKyD7Hyu2fpBUneamLMBXmP3qsYX6HoTw7yonpjWyC")));
    ...
    // failed address
    asset ast;
    READ_DB_ASSET(address(public_key_type(std::string("VAST7rbe5ZqAEtwQT6Tw39R29vojFqrCQasK3nT5s2pEzXh1BABXHF"))), symbol(5, get_sym_id()), ast);
    CHECK(ast.amount() == 500000);
