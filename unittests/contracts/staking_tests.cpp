...
TEST_CASE_METHOD(contracts_test, "staketkns_test", "[contracts][staking]") {
    ...
    my_tester->add_money(key, asset(10'000'000'00000, vast_sym()));
    ...
    stakepool_def stakepool_;
    READ_TOKEN(stakepool, 1, stakepool_);
    CHECK(stakepool_.total == asset(500000'00000, vast_sym()));
    ...
    READ_TOKEN(stakepool, 1, stakepool_);
    CHECK(stakepool_.total == asset(1'000'000'00000, vast_sym()));
...
TEST_CASE_METHOD(contracts_test, "unstaketkns_test", "[contracts][staking]") {
    ...
    READ_DB_ASSET(unstk.staker, vast_sym(), prop);
    ...
    auto vaddr = address(N(.validator), validator_.name, VAST_SYM_ID);
    auto vprop = property();
    READ_DB_ASSET(vaddr, vast_sym(), vprop);
...
TEST_CASE_METHOD(contracts_test, "valiwithdraw_test", "[contracts][staking]") {
    ...
    {
      "name": "validator",
      "addr": "VAST546WaW3zFAxEEEkYKjDiMvg3CHRjmWX2XdNxEhi69RpdKuQRSK",
      "amount": "10.00000 S#1"
    }
    )=====";
