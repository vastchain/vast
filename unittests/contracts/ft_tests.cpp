...
TEST_CASE_METHOD(contracts_test, "newfungible_test", "[contracts]") {
    const char* test_data = R"=====(
    {
      ...
      "issue" : {
        ...
        "threshold" : 1,
        "authorizers": [{
            "ref": "[A] VAST6NPexVQjcb2FJZJohZHsQ22rRRtHziH8yPfyj2zwnJV74Ycp2p",
            "weight": 1
          }
        ]
      },
    ...
    my_tester->add_money(fungible_payer, asset(10'000'000, symbol(5, VAST_SYM_ID)));
...
TEST_CASE_METHOD(contracts_test, "newfungible_v2_test", "[contracts]") {
    ...
    auto var            = fc::json::from_string(test_data);
    auto fungible_payer = address(N(.domain), ".fungible", 0);
    my_tester->add_money(fungible_payer, asset(10'000'000, symbol(get_sym_id(2), VAST_SYM_ID)));
    auto& tokendb = my_tester->control->token_db();
    auto& cache = my_tester->control->token_db_cache();
...
TEST_CASE_METHOD(contracts_test, "transferft_test", "[contracts]") {
    const char* test_data = R"=====(
    {
      "from": "VAST6NPexVQjcb2FJZJohZHsQ22rRRtHziH8yPfyj2zwnJV74Ycp2p",
    ...
    my_tester->add_money(payer2, asset(100'000'000, vast_sym()));
...
TEST_CASE_METHOD(contracts_test, "vast2pvast_test", "[contracts]") {
    const char* test_data = R"=======(
    {
    ...
    e2p.from = payer;
    to_variant(e2p, var);
    CHECK_THROWS_AS(my_tester->push_action(N(vast2pvast), N128(.fungible), (name128)std::to_string(vast_sym().id()), var.get_object(), key_seeds, payer), asset_symbol_exception);

    e2p.number = asset::from_string(string("5.00000 S#1"));
    e2p.to = address();
    to_variant(e2p, var);
    CHECK_THROWS_AS(my_tester->push_action(N(vast2pvast), N128(.fungible), (name128)std::to_string(vast_sym().id()), var.get_object(), key_seeds, payer), address_reserved_exception);
    ...
    e2p.number = asset::from_string(string("5.00000 S#1"));
    to_variant(e2p, var);
    my_tester->push_action(N(vast2pvast), N128(.fungible), (name128)std::to_string(vast_sym().id()), var.get_object(), key_seeds, payer);

    asset ast;
    READ_DB_ASSET(key, pvast_sym(), ast);
    ...
    to_variant(tf, var);
    CHECK_THROWS_AS(my_tester->push_action(N(transferft), N128(.fungible), (name128)std::to_string(pvast_sym().id()), var.get_object(), key_seeds, payer), unsatisfied_authorization);
