...
TEST_CASE_METHOD(contracts_test, "charge_test", "[contracts]") {
    ...
    READ_DB_ASSET(prod, vast_sym(), prodasset_before);
    ...
    READ_DB_ASSET(prod, vast_sym(), prodasset_after);
...
TEST_CASE_METHOD(contracts_test, "addmeta_test", "[contracts]") {
    my_tester->add_money(payer, asset(10'000'000, symbol(5, VAST_SYM_ID)));
    ...
    const char* domain_data = R"=====(
        {
          "name" : "gdomain",
          "creator" : "VAST5ve9Ezv9vLZKp1NmRzvB5ZoZ21YZ533BSB2Ai2jLzzMep6biU2",
          "issue" : {
            "name" : "issue",
            "threshold" : 1,
            "authorizers": [{
                "ref": "[A] VAST5ve9Ezv9vLZKp1NmRzvB5ZoZ21YZ533BSB2Ai2jLzzMep6biU2",
                "weight": 1
              }
            ]
          },
    ...
    const char* tk_data = R"=====(
    {
      "domain": "gdomain",
        ...
        "owner": [
          "VAST5ve9Ezv9vLZKp1NmRzvB5ZoZ21YZ533BSB2Ai2jLzzMep6biU2"
        ]
    }
    )=====";
...
TEST_CASE_METHOD(contracts_test, "updsched_test", "[contracts]") {
    ...
    us.producers[0].block_signing_key = tester::get_public_key("vast");
    to_variant(us, var);
