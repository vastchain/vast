...
TEST_CASE_METHOD(contracts_test, "issuetoken_test", "[contracts]") {
    ...
    my_tester->add_money(address(N(.domain), name128(get_domain_name()), 0), asset(10'000'000, symbol(5, VAST_SYM_ID)));
...
TEST_CASE_METHOD(contracts_test, "transfer_test", "[contracts]") {
    ...
    trf.to.clear();
    trf.to.emplace_back("VAST8MGU4aKiVzqMtWi9zLpu8KuTHZWjQQrX475ycSxEkLd6aBpraX");
    trf.to.emplace_back("VAST6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV");
    to_variant(trf, var);
