...
const char* domain_data = R"=====(
    {
     ...
     "transfer": {
      ...
      "metas":[{
      	"key": "key",
      	"value": "value",
      	"creator": "[A] VAST546WaW3zFAxEEEkYKjDiMvg3CHRjmWX2XdNxEhi69RpdKuQRSK"
      }]
    }
    )=====";
...
TEST_CASE_METHOD(tokendb_test, "put_tokens_test", "[tokendb]") {
    ...
    auto tkeys = vast::chain::token_keys_t();
    tkeys.push_back(tk1.name);
    tkeys.push_back(tk2.name);
    
    auto data = small_vector<std::string_view, 4>();
    data.push_back(vast::chain::make_db_value(tk1).as_string_view());
    data.push_back(vast::chain::make_db_value(tk2).as_string_view());

    tokendb.put_tokens(
            vast::chain::token_type::token,
            vast::chain::action_op::put,
            "dm-tkdb-test",
            std::move(tkeys),
            data);
