...
class tokendb_test {
public:
    //tokendb_test() : tokendb(vast_unittests_dir + "/tokendb_tests") {
    tokendb_test() {
        auto basedir = vast_unittests_dir + "/tokendb_tests";
...
#define EXISTS_TOKEN(TYPE, NAME) \
    tokendb.exists_token(vast::chain::token_type::TYPE, std::nullopt, NAME)

#define EXISTS_TOKEN2(TYPE, DOMAIN, NAME) \
    tokendb.exists_token(vast::chain::token_type::TYPE, DOMAIN, NAME)
...
#define READ_TOKEN(TYPE, NAME, VALUEREF) \
    { \
        auto str = std::string(); \
        tokendb.read_token(vast::chain::token_type::TYPE, std::nullopt, NAME, str); \
        vast::chain::extract_db_value(str, VALUEREF); \
    }

#define READ_TOKEN2(TYPE, DOMAIN, NAME, VALUEREF) \
    { \
        auto str = std::string(); \
        tokendb.read_token(vast::chain::token_type::TYPE, DOMAIN, NAME, str); \
        vast::chain::extract_db_value(str, VALUEREF); \
    }
...
#define ADD_TOKEN(TYPE, KEY, DATA) \
    { \
        auto dbvalue = vast::chain::make_db_value(DATA); \
        tokendb.put_token(vast::chain::token_type::TYPE, vast::chain::action_op::add, std::nullopt, KEY, dbvalue.as_string_view()); \
    }

#define UPDATE_TOKEN(TYPE, KEY, DATA) \
    { \
        auto dbvalue = vast::chain::make_db_value(DATA); \
        tokendb.put_token(vast::chain::token_type::TYPE, vast::chain::action_op::update, std::nullopt, KEY, dbvalue.as_string_view()); \
    }

#define PUT_TOKEN(TYPE, KEY, DATA) \
    { \
        auto dbvalue = vast::chain::make_db_value(DATA); \
        tokendb.put_token(vast::chain::token_type::TYPE, vast::chain::action_op::put, std::nullopt, KEY, dbvalue.as_string_view()); \
    }

#define ADD_TOKEN2(TYPE, DOMAIN, KEY, DATA) \
    { \
        auto dbvalue = vast::chain::make_db_value(DATA); \
        tokendb.put_token(vast::chain::token_type::TYPE, vast::chain::action_op::add, DOMAIN, KEY, dbvalue.as_string_view()); \
    }

#define UPDATE_TOKEN2(TYPE, DOMAIN, KEY, DATA) \
    { \
        auto dbvalue = vast::chain::make_db_value(DATA); \
        tokendb.put_token(vast::chain::token_type::TYPE, vast::chain::action_op::update, DOMAIN, KEY, dbvalue.as_string_view()); \
    }

#define PUT_TOKEN2(TYPE, DOMAIN, KEY, DATA) \
    { \
        auto dbvalue = vast::chain::make_db_value(DATA); \
        tokendb.put_token(vast::chain::token_type::TYPE, vast::chain::action_op::put, DOMAIN, KEY, dbvalue.as_string_view()); \
    }
