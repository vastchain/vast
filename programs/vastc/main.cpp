...
using namespace vast::utilities;
using namespace vast::client::help;
using namespace vast::client::http;
using namespace vast::client::localize;
using namespace vast::client::config;
using namespace boost::filesystem;

FC_DECLARE_EXCEPTION(explained_exception, 9000000, "explained exception, see error log");
FC_DECLARE_EXCEPTION(localized_exception, 10000000, "an error occured");

#define VASTC_ASSERT(TEST, ...)                                \
    FC_EXPAND_MACRO(                                          \
...
string program            = "vastc";
string url                = "http://127.0.0.1:8888";
string default_wallet_url = "unix://" + fc::path(determine_home_directory() / "vast-wallet/vastwd.sock").to_native_ansi_path();
...
print_result(const fc::variant& result) {
    try {
        if(result.is_object() && result.get_object().contains("processed")) {
            ...
            cerr << "total charge: " << asset(processed["charge"].as_int64(), vast_sym()) << std::endl;
...
vast::client::http::http_context context;
...
call(const std::string& url,
    ...
    try {
        vast::client::http::connection_param *cp = new vast::client::http::connection_param(context, parse_url(url) + path,
            no_verify ? false : true, headers, raw_response);

        return vast::client::http::do_http_call(*cp, fc::variant(v), print_request, print_response);
    }
    catch(boost::system::system_error& e) {
        if(url == ::url)
            std::cerr << localized("Failed to connect to vastd at ${u}; is vastd running?", ("u", url)) << std::endl;
        else if(url == ::wallet_url)
            std::cerr << localized("Failed to connect to vastwd at ${u}; is vastwd running?", ("u", url)) << std::endl;
...
set_execution_context(execution_context& exec_ctx) {
    auto acts = call(get_vast_actions, fc::variant()).as<std::vector<action_ver_type>>();
    for(auto& av : acts) {
...
vast::chain_apis::read_only::get_info_results
get_info() {
    return ::call(url, get_info_func, fc::variant()).as<vast::chain_apis::read_only::get_info_results>();
}
...
void
set_transaction_header(signed_transaction& trx, const vast::chain_apis::read_only::get_info_results& info) {
    ...
    VAST_RETHROW_EXCEPTIONS(invalid_ref_block_exception, "Invalid reference block num or id: ${block_num_or_id}", ("block_num_or_id", tx_ref_block_num_or_id));
...
push_transaction(signed_transaction& trx, packed_transaction::compression_type compression = packed_transaction::none) {
    ...
    if(get_charge_only) {
        ...
        return fc::variant(asset(c["charge"].as_int64(), vast_sym()));
    }
...
try_local_port(uint32_t duration) {
    ...
    while(!local_port_used()) {
        if(duration_cast<std::chrono::milliseconds>(system_clock::now().time_since_epoch()).count() - start_time > duration) {
            std::cerr << "Unable to connect to vastwd, if vastwd is running please kill the process and try again.\n";
            throw connection_exception(fc::log_messages{FC_LOG_MESSAGE(error, "Unable to connect to vastwd")});
        }
    }
}

void
ensure_vastwd_running(CLI::App* app) {
    // version, net do not require vastwd
...
next:
    ...
    // This extra check is necessary when running vastc like this: ./vastc ...
    if(bin_path.filename_is_dot()) {
        bin_path.remove_filename();
    }
    bin_path.append("vastwd"); // if vastc and vastwd are in the same installation directory
    if(!boost::filesystem::exists(bin_path)) {
        bin_path.remove_filename().remove_filename().append("vastwd").append("vastwd");
    }
    ...
    if(boost::filesystem::exists(bin_path)) {
        ...
        pargs.push_back(fc::path(determine_home_directory() / "vast-wallet/vastwd.sock").to_native_ansi_path());

        // namespace bp = boost::process;
        // ::boost::process::child vastwd(binPath, pargs,
        //                               bp::std_in.close(),
        //                               bp::std_out > bp::null,
        //                               bp::std_err > bp::null);
        // if(vastwd.running()) {
        ...
        if((pid = fork()) == 0) {
            ...
            std::cerr << "Cannot fork to start vastwd" << std::endl;
        }
        else {
        ...
        std::cerr << "No wallet service listening on " << wallet_url
                  << ". Cannot automatically start vastwd because vastwd was not found." << std::endl;
    }
}
...
auto parse_permission = [](auto& jsonOrFile) {
    ...
    VAST_RETHROW_EXCEPTIONS(permission_type_exception, "Fail to parse Permission JSON")
};
...
auto parse_group = [](auto& jsonOrFile) {
    ...
    VAST_RETHROW_EXCEPTIONS(group_type_exception, "Fail to parse Group JSON")
};
...
struct set_fungible_subcommands {
    ...
    set_fungible_subcommands(CLI::App* actionRoot) {
        ...
        nfcmd->callback([&] {
            ...
            VAST_ASSERT(nf.total_supply.sym() == nf.sym, asset_type_exception, "Symbol and asset should be match");
...
struct set_assets_subcommands {
    ...
    set_assets_subcommands(CLI::App* actionRoot) {
        ...
        auto epcmd = actionRoot->add_subcommand("2pvast", localized("Convert VAST tokens to Pinned VAST tokens"));
        ...
        epcmd->callback([this] {
            ...
            FC_ASSERT(ep.number.sym() == vast_sym(), "Only VAST can be converted to Pinned VAST");
        ...
        stkcmd->callback([this] {
            ...
            FC_ASSERT(stk.amount.sym() == vast_sym(), "Only VAST is supported to stake currently");
        ...
        unstkcmd->callback([this] {
            ...
            FC_ASSERT(unstk.sym_id == vast_sym().id(), "Only VAST is supported to unstake currently");
        ...
        tatkcmd->callback([this] {
            ...
            FC_ASSERT(tatk.sym_id == vast_sym().id(), "Only VAST is supported to stake currently");
...
struct set_suspend_subcommands {
    ...
    set_suspend_subcommands(CLI::App* actionRoot) {
        ...
        adcmd->callback([this] {
            ...
            auto exec_ctx = vast_execution_context_mock();
            set_execution_context(exec_ctx);

            auto abi = abi_serializer(vast_contract_abi(), std::chrono::hours(1));
...
struct set_get_vastlink_signed_keys_subcommand {
    string link_id;

    set_get_vastlink_signed_keys_subcommand(CLI::App* actionRoot) {
        auto gsscmd = actionRoot->add_subcommand("vastlinkkeys", localized("Retrieve Vastlink signed keys"));
        gsscmd->add_option("linkid", link_id, localized("id of Vastlink"))->required();

        gsscmd->callback([this] {
            auto arg = fc::mutable_variant_object("link_id", link_id);
            print_info(call(get_vastlink_signed_keys_func, arg));
        });
    }
};
...
class VASTApp : public CLI::App {
public:
    VASTApp(std::string app_description = "", std::string app_name = "")
        : CLI::App(app_description, app_name) {}

public:
    void
    pre_callback() override {
        ensure_vastwd_running(this);
    }
};
...
main(int argc, char** argv) {
    ...
    app.add_option("--wallet-url", wallet_url, localized("the http/https/unix-socket URL where vastwd is running"))->capture_default_str();
    ...
    version->add_subcommand("client", localized("Retrieve version information of the client"))->callback([] {
        std::cout << localized("Build version: ${ver}", ("ver", vast::client::config::version_str)) << std::endl;
    });
    ...
    get->add_subcommand("actions", localized("Get current actions"))->callback([] {
        std::cout << fc::json::to_pretty_string(call(get_vast_actions, fc::variant())) << std::endl;
    });

    // get abi
    get->add_subcommand("abi", localized("Get current ABI"))->callback([] {
        std::cout << fc::json::to_pretty_string(call(get_vast_abi, fc::variant())) << std::endl;
    });
    ...
    set_get_vastlink_signed_keys_subcommand get_vastlink_signed_keys(get);
    ...
    importWallet->callback([&wallet_name, &wallet_key_str] {
        ...
        try {
            ...
            VAST_THROW(private_key_type_exception, "Invalid private key: ${private_key}", ("private_key", wallet_key_str))
        }
    ...
    removeKeyWallet->callback([&wallet_name, &wallet_pw, &wallet_rm_key_str] {
        ...
        try {
            ...
            VAST_THROW(public_key_type_exception, "Invalid public key: ${public_key}", ("public_key", wallet_rm_key_str))
        }
    ...
    // stop vastwd
    auto stopVastwd = wallet->add_subcommand("stop", localized("Stop vastwd (doesn't work with vastd)."));
    stopVastwd->callback([] {
        const auto& v = call(wallet_url, vastwd_stop);
        if (!v.is_object() || v.get_object().size() != 0) { //on success vastwd responds with empty object
    ...
    sign->callback([&] {
        ...
        if(str_chain_id.size() == 0) {
            ilog("grabbing chain_id from vastd");
            auto info = get_info();
    ...
    trxSubcommand->callback([&] {
        ...
        VAST_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON")
